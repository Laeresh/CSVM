...
**No active plan as of 2026-07-30** — every plan sits in [`docs/plans/plans.md`](docs/plans/plans.md). **Next:** pick the next item from `backlog.md`, or scaffold a new plan with `/new-plan`. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
