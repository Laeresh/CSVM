...
- `src/Testing/` (4) — the in-engine assertion harness behind `--run-tests` and the `--dump-*` probes.
...
**No active plan** — every plan is complete, indexed in [`docs/plans/plans.md`](docs/plans/plans.md); `/new-plan` scaffolds the next one. Verify a change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
