...
**Active plan: [`docs/PLAN-sessionspec.md`](docs/PLAN-sessionspec.md)** — Wave A complete, Wave B at B7, next **B8** (the last item). Verify a change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
