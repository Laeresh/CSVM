...
**Next step: `docs/PLAN-M2-polish-4.md` wave D (item 8, the crash choreography) and wave E (items 9 and 10, rendering).** Waves A, B and C are complete as of 2026-07-22 — items 1, 2, 4, 5 and 7 landed, item 3 moved back to `backlog.md`, and item 6 is the only one of the six left open. **Item 9 is mostly CLOSED**: C1B was ruled not-a-bug by the user (it is in the original), which retires the dense-rank direction it was built around; only C5's within-node `SurfaceRankBias` conflict may survive, and only if the original does not z-fight there either — unconfirmed, and it gates any work. Item 10 has `SceneBuilder.cs` to itself as a result. **One landed change is user-vetoable:** the pad-read-on-focus gate shipped as its own commit alongside item 7's focus mute; reverting it alone leaves the mute intact, and it will stop a pad working while the window is unfocused. Separately, **the owed playtests** remain the real blocker on calling Milestone 2.5 done, and they need the user at the controls — several need **two controllers**, which this machine does not have. Both lists live in `backlog.md`: "Owed playtests" and "TUNE constants pending playtest".

**Known issues — diagnosed, unscheduled.** Full diagnoses are in `backlog.md` so they are not re-chased:

- **C1B z-fighting is NOT A BUG — user ruling 2026-07-22: "it's in the original."** Do not fix it, and **do not raise `NodeOrderBias`**: the measured 5e-8 → 2e-6 control taking C1B 30.95%→2.35% was never an improvement, it was 1.4 points from erasing a faithful artifact. This also retires the **conflict-local / dense-rank direction entirely**, because C1B was the only half it could fix. Four wrong mechanisms were retired getting here and all four are recorded so they are not re-chased (`analysis/item9-depth-bias/FINDINGS.md`, `docs/PLAN-M2-polish-4.md` item 9): it is *not* coarse-sheet-vs-partition-ground, *not* `g4683` fighting its own polygons, *not* fixable by a per-polygon within-surface tie-break, and the "nine coplanar World-child nodes" premise is an AABB reading — **they tile, 0.00 m² true overlap.** What is measured and survives: **the two poses were always two different bugs.** C5 = `g4683` fighting its own surfaces (separation is `SurfaceRankBias` alone; `node_bias` cancels, so no per-node scheme can reach it); C1B = one cross-node pair, now ruled authentic. **C5 IS a real defect — and is not a depth-bias bug either (fifth mechanism, user observation 2026-07-22).** The original runs a **ground-texture LOD chain** (`cblock1_2` → `cblock1_1` → `cblock1`, dark-with-points → brightly-lit street) and **shows no `cblock[4-6]` in C5 IA1 at all** — while our 78% conflict is `cblock4` over `cblock2` over `cblock1`, coplanar inside `g4683`. So we render ground variants the original never draws; the flicker is a *symptom* of drawing 2–3 layers where it draws 1. **⛔ Do NOT raise `SurfaceRankBias` even though it measures perfectly** (0.00%) — it picks a winner among surfaces that should not be co-rendered, i.e. a flawless metric over a wrong picture (`docs/verification.md` rules 4 and 8). The real lead is **partition visibility** (`WorldPartitionSetActive`, 25 uses in interp), which `backlog.md` already describes as the original's coarse/fine ground selector that we ignore — item 9 named it and set it aside as unrelated. C3's coast is **fixed** (`6c592c2`; 4.87%→0.09%) — do not re-chase it.
