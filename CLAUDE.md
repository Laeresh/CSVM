...
**Active plan: [`docs/PLAN-sessionspec.md`](docs/PLAN-sessionspec.md)** — Wave A, next **A4** (the gate). Verify a change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
