...
**The active plan is now [`docs/PLAN-M3-weapons.md`](docs/PLAN-M3-weapons.md)** (written 2026-07-22, the only plan still in `docs/`) — Milestone 3, weapons and destruction: 44 items in six waves, scope settled with the user. **No item is user-gated** — both user-owned inputs (the gun mount-name table and the `CLUSTER_SIZE` playtest) are recorded in the plan. Wave A (research/docs) is the place to start.
