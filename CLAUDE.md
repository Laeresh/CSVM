...
**Next step: `docs/PLAN-M2-polish-4.md` wave A item 2** — `FromToMotion` resets orientation to the authored rest pose, so C1's cars drive mis-headed. Item 1 (the sequence-scheduler off-by-one) landed 2026-07-22 and item 3 moved back to `backlog.md`, so item 2 is what remains in wave A. **Re-baseline the police chase before starting it** — item 1 changed when every event fires, so any capture taken before that is stale. Separately, **the owed playtests** remain the real blocker on calling Milestone 2.5 done, and they need the user at the controls — several need **two controllers**, which this machine does not have. Both lists live in `backlog.md`: "Owed playtests" and "TUNE constants pending playtest".
