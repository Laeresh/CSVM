...
**Active plan: [`docs/PLAN-m3-polish-quickwins.md`](docs/PLAN-m3-polish-quickwins.md)** (started 2026-07-30) — 11 backlog items (quick-win/code-verifiable + user-added `BL-025`) in waves A/B/C/D; waves A and B complete. **Next:** wave C — C21 (`BL-139`), C22 (`BL-159`). Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
