...
**Next step: `docs/PLAN-M2-polish-4.md` wave D (item 8, the crash choreography) and wave E (items 9 and 10, rendering).** **Waves A, B and C are COMPLETE** (items 1, 2, 4, 5, 6, 7 landed 2026-07-22/23; item 3 moved back to `backlog.md`), and all six were verified together in a cross-item regression. **Item 9 is closed as a depth problem**: C1B is not a bug (user ruling — it is in the original), and C5's mechanism is now found and is the **unparsed subface flag**, ~10 lines to fix and *not* a bias change — that is the highest-value work left in the plan. Item 10 has `SceneBuilder.cs` to itself, but **must be sequenced with the subface fix** (both touch the surface-group key / bias path). **One landed change is user-vetoable:** the pad-read-on-focus gate shipped as its own commit alongside item 7's focus mute; reverting it alone leaves the mute intact, and it will stop a pad working while the window is unfocused. Separately, **the owed playtests** remain the real blocker on calling Milestone 2.5 done, and they need the user at the controls — several need **two controllers**, which this machine does not have. Both lists live in `backlog.md`: "Owed playtests" and "TUNE constants pending playtest".

**Known issues — diagnosed, unscheduled.** Full diagnoses are in `backlog.md` so they are not re-chased:

- **Ground z-fighting — settled 2026-07-22 after FIVE wrong mechanisms; read `backlog.md`'s "Partition / ground-variant visibility" entry before touching it.** Two rulings, both from the user: **C1B is NOT a bug** (it is in the original — do not fix it, and **never raise `NodeOrderBias`**; the 30.95%→2.35% control was 1.4 points from erasing a faithful artifact). **C5 IS a defect, is not a depth bug, and the mechanism is now FOUND (2026-07-23): we never parse the OpenFlight *subface* flag.** Polygon flag `unk3` (`0x0800`) marks "coplanar with and contained in the face beneath — draw on top", and `support\init.gw` applies `GameGenSetSubfacePriorityOffset 1` to it globally; `grep unk3 CSVM/src` → 0 hits. `cblock4/5/6` are the **base** ground, `cblock1/2/3` are subfaces on it. Measured: 25.85M m² of C5 ground is subface-over-base and we render **66.5% inverted**; applying the flag takes the repro pose **78.14%→0.00%** with C1B and C3 **byte-identical** (C1B has zero `unk3` polygons — independent support for the not-a-bug ruling). **Fix is 3 edits/~10 lines at `GameZ.cs:278`, `SceneBuilder.cs:386` and `:841`, offset 1e-4, and NO bias-constant changes** — ⛔ still do not raise `SurfaceRankBias`. Regression risk is rank shifting, so the 8-chapter surface-count run is the check. Evidence + instrument: `analysis/item9-depth-bias/CBLOCK-LOD.md`. C3's coast is **fixed** (`6c592c2`) — do not re-chase it.
