...
- **Format reverse engineering:** happens in a fork of [mech3ax](https://github.com/TerranMechworks/mech3ax) (Rust) at `tools/mech3ax/`. Their byte-identical round-trip test harness (extract→repack) is the correctness standard. **The CS work stays in the fork** — see "The mech3ax fork" below.
...
## The mech3ax fork (`tools/mech3ax/`, git-ignored)

**Crimson Skies support lives in the fork, not upstream** (decided 2026-07-22): upstream removed it because they could not maintain it — bandwidth, not architecture — so the fork is its home, and upstream commits get merged *into* the fork if they appear. Upstream is **dormant** (`cbb838f`, rc3, 2025-11-17; `upstream/main` 0 commits ahead of fork `main` as of 2026-07-22), so syncing is a check-occasionally, not a routine.

- Remotes: `origin` = `git@github.com:Laeresh/mech3ax.git`, `upstream` = `https://github.com/TerranMechworks/mech3ax.git`.
- Branch roles, **all three pushed to `origin`** (the fork is the authoritative copy, not this workstation): `cs-anim` = integration + what the release binary is built from; `pr-cs-anim` / `pr-cs-gamez` = the same work split by concern; `main` = an upstream mirror.
- Sync: `git fetch upstream && git merge upstream/main` into the fork, then rebuild the release binary `ExtractAssets.ps1` uses (`tools/mech3ax/target/release/unzbd.exe`) and re-extract if anything in the output shape moved.
- The shelved upstream-contribution package is archived at `docs/plans/upstream-pr/` — kept because its PR bodies are the best description of what each branch contains.
...
- `docs/plans/` — **completed plans, archived** (each banner-marked `COMPLETE` with its date): the two M2 polish runs, M2.5, the mech3ax-fork revival, animation playback, and the anim/rendering follow-ups — plus `upstream-pr/`, the shelved upstream-contribution package. Kept for their evidence, measurements and recorded dead ends; read them as history. The status table below lists each with its scope and date. **The one plan NOT in here is the active one** — that is the signal.
...
| `docs/PLAN-docs-cleanup.md` | Shrink this file back to an index (10 items) | ✅ 2026-07-22 |

**No plan is active.** `docs/` holds one plan file and `docs/plans/` holds the rest; when the next plan is written it becomes the obvious single live document.
...
**Next step: the owed playtests.** They are the real blocker on calling Milestone 2.5 done, and they need the user at the controls — several need **two controllers**, which this machine does not have. Both lists live in `backlog.md`: "Owed playtests" and "TUNE constants pending playtest".
