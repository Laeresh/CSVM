...
- `horizon` subtree = original skydome (~8.8 km radius, verts centered on the **origin** while the world area is x,z ∈ [-12288, 0] — the engine must translate it with the viewer; it's a camera-anchored backdrop, not scenery). Two day/night zone variants as child nodes: `zone1` (day) = `h_zone1scroll` (sky2.tif tiling haze dome, u tiles ~22× around, sunset-orange vertex tints near the horizon) + `o28` (flat-colored gray sea-haze skirt, y −2000..271); `zone2` (night) = `moon` (moon1.tif quad) + `g1155`/`h_zone2scroll` (Sky1.tif sky-to-cloudbank band, dark blue vertex tints) + `stars` (an **empty** mesh). Which zone a mission uses isn't in the zrdr readers (likely engine-side); C1 free flight uses zone1. The `*scroll` names imply the original scrolls the sky UVs — rate unknown, not implemented.
...
Remaining known gaps: sky UV scroll animation not implemented (`h_zone1scroll` — rate unknown), zone2 night sky unused, clouds are alpha-scissor cutouts (original alpha-blends; they read as hard gray facets when flying at deck height), prop/vehicle animations static, no propstart/propstop one-shots (mid-air spawn starts with engine running), no crash explosion visual (sound + hide only). With rendering, flight, sound, collision, and the skydome in place, **Milestone 2's core is essentially complete.** **Next step:** pick among the remaining polish items (candidates: crash explosion visual using the surveyed flame_ball.json emitters + fire flipbook textures, or alpha-blended clouds).
