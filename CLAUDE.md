...
**The active plan is [`docs/PLAN-M3-weapons.md`](docs/PLAN-M3-weapons.md)** (written 2026-07-22) — Milestone 3, weapons and destruction, 44 items in six waves. Wave A's docs are done (A1/A2/A4/A5/A6 → `weapons.md`, `markers.md`, `destructibles.md`, `weapon-effects.md`, `vehicle.md`); A3 landed (the `--dump-markers` tool + the `--viewer --markers` overlay, key K); A7 landed (`CSVM/data/stock_loadouts.json` + `docs/formats/loadouts.md`). **Wave A is complete bar A10** (the in-engine firing-placement check, which waits on D29). **Wave B is underway — guns fire.** Landed: B11 (`WeaponDefs.cs` — `--dump-weapons`), B12 (`Loadout.cs` — `--dump-loadout`), the **B13/B15/B16/D29/D30/D33 batch** (hold **Space**/pad-B: per-group ammo at the data's `FIRE_RATE`, `CANNON_SPREAD`, velocity-aligned tracers, muzzle flash, world-raycast impacts classified by surface — terrain/water/buildings — with the `IMPACT` sound); and **B17** (rockets): **F** (pad A) launches **one HE rocket per pull** from the next armed pylon round-robin, gated by the rocket's `FIRE_RATE 1.0` (1 s) cooldown, depleting each pylon's own `CLUSTER_SIZE` counter (stock Bloodhawk = 3 pylons × 3 = 9, verified). and **B18** (selectors): **G**/D-pad-L cycles which gun group fires — **one at a time, no ALL** (user-corrected: the original never fires all groups at once); **H**/D-pad-R cycles ordnance (stock = one HE type, so a no-op until mixed loadouts); and **B14** (rockets fly the `FLYOUT` MODEL body — each launched rocket instances its `.flt` prototype root (`he_rocket` …) from the chapter gamez via the world `SceneBuilder`, collision-exempt, posed nose-forward; guns keep the tracer quad — measured; the `MODEL_ANIMATION` smoke trail stays D-wave). `--fire`/`--fire-rockets`/`--infinite-ammo`/`--loadout=`/`--gun-select=N` for testing. **Guided flight (B19/B20) is deferred to M4** (decided 2026-07-24): the original has no manual ground-target selection — its auto-aim is game-handled and enemy-plane-only — and M3 has no enemy planes to lock, so **every rocket, the Seeker included, fires as dumbfire** (the ballistic pool it already flies through; no code change). With B14 landed, **Wave B's remaining in-scope work is the D32 named-effect / per-ammo-tracer polish (D30's impact-effect models now landed). Wave C (destruction) is COMPLETE: C21–C28 landed** (2026-07-24). C21 gives every `HEALTH>0` world object a live mutable HP pool (`src/Mech3/DestructibleRegistry.cs`, one per `(def,anchor)`) that `ANIM_HEALTH` reads; C22 parses the `DAMAGE_SEQUENCE` (reader + compiled) and runs it against that live HP (`AnimRuntime.ApplyDamageStages`), firing each progressive stage effect once at its authored threshold; C23 makes **weapon fire damage destructibles** — `ProjectilePool.Impact` → `AnimRuntime.DamageAt` resolves the struck collider (`Registry.Resolve`, compiled-preferred walk-up), spends `HEALTH_DAMAGE` (world objects carry HEALTH only, no armour pool), and marks it destroyed at zero (HE 1-hit / AP 2-hit a HEALTH-60 tower); C24 makes shot objects **die** — at zero HP `RunDeathSequence` plays the def's death via `Start` (the healthy→destroyed swap + puffers; a RESET-derived swap fallback covers the ~10% of defs that author none), verified by the `--damage-hd` swap check (all 16 C1 destructibles hide healthy + show their wreck); C25 needed **no new code** — that swap runs `SetSubtreeActive`, which toggles colliders with visibility, so death removes the healthy door/building collision and solids the wreck for free (proven by a `col[off,on]` census: C2 doors off 1/on 8; the Hollywood **propane→door chain works** — shooting the collidable `propane` tank swaps `kkgate` and chains the bridge fires). C26 also needed **no new code**: the ballistic `OBJECT_MOTION` debris path was already built by the M2 crash `MotionRuntime` and is reached on death (the death's motion events are `Initial`, so C24's `Start` runs them) — the C24 "stubbed" note was a harness artifact (the kill-and-check never advanced the clock past the debris's scheduled t≈2.2 s launch); verified by `--damage-hd` `debris[N]` (water tower flings 2 visible pieces, buildings 7, no-motion deaths 0). C27 wires **collision** into destruction (`FlightController` → `AnimRuntime.CollideDamageAt`, gated on `ACTIVATION`: the 44 `WeaponOrCollideHit` objects — C2 facades `fcpan*`, C5 windows `w_win*` at 0.01, C5 `agyrobus` at 70 — break on contact `vn × 8` and the plane flies through, while `WeaponHit` objects stay solid, decision 6); and C28 adds **reset/restore** (`AnimRuntime.ResetDestructible`: Stop the death, restore the flown debris to rest, re-apply RESET_STATE, restore HP — destroy→reset→destroy idempotent, for the debug tools + respawn). C21–C28 are **no-ops at world build**, confirmed by a clean 8-chapter freecam regression byte-identical to before, and are exercised by the `--damage-hd` harness (`swap`/`col[off,on]`/`debris[N]`/`collide[✓/✗]`/`reset[…]`). **Wave C (destruction) is complete** — objects damage, die, lose+gain collision, throw debris, break on contact, and reset. **Wave D is underway.** Landed: D29 (muzzle flash), D33 (tracers), and **D30** (impact effects, 2026-07-24): `ProjectilePool` instances the per-surface `IMPACT` effect **model** at the hit point when the bound name is a chapter-gamez root — the water splash (`splash1.flt` guns / `bsplsh.flt` HE, verified C1B/C2B) — plus the per-surface sound and a spark fallback; the 5 undefined effect names confirmed inert. **D30 corrected the plan's premise:** the **puffer** half of the named effects (`gunhit` smoke, fireballs) can't build at runtime in flight (the puffer factory is torn down after the world build; `verification.md` rule 76), so it **folds into D32** — a world-effects runtime generalizing `BuildFlightCrashRuntime` that both impact puffers and destruction effects share. And **D31** (the `Sound` anim-event family, 2026-07-24): `AnimRuntime.HandleSound` plays the one-shot `SOUND` (death/damage/impact audio) as a fire-and-forget `WorldSounds.PlayOneShot`, resolving `SOUND_GROUPS` through the new `DYNAMIC_WEIGHTS` decode (`SoundDefs.LoadGroups`); verified across all 8 chapters (details in `docs/HISTORY.md`). **Next in Wave D:** D32 (that world-effects runtime), D44 (pylon ordnance visuals); plus A10 (in-engine flash-placement check) and the owed in-flight playtests. Still deferred out of Wave C: the debris **ground-rest** (`do_intersections`, a Layer-1.5 follow-up). **Owed playtests** are consolidated in [`playtest.md`](playtest.md) — the in-flight visual of a destructible dying (wreck + debris tumble), flying through a killed door, and flying through a facade vs. crashing into a tower.
