...
**Four plan files sit in `docs/`. [`docs/PLAN-anim-debugger.md`](docs/PLAN-anim-debugger.md) is now banner-marked COMPLETE (Waves 1–5, landed 2026-07-23; archive to `docs/plans/` on the next cleanup)** — the permanent `--anim-lab` mode: the `PlaneViewer` split, additive runtime capabilities, the MVP, the def picker + authored-vs-fired timeline, and Wave 5's stage-placeless-defs-in-front-of-the-camera (which also delivered the crash plan's Wave 2a/2b scaffolding + the template-instancing `PlaceCalledTemplates`). **The active plan is now [`docs/PLAN-data-driven-crash.md`](docs/PLAN-data-driven-crash.md)** — its **Layer 1 (Wave 1) and Layer 2 (Waves 2–3) both landed 2026-07-23**, Layer 2 gated behind **`--data-crash`** (default off; the bespoke `CrashChoreography`/`CrashBreakup` stay the verified fallback for the A/B). With the flag, a crash PLAYS the compiled `player_crash_dirt` def on a **per-player scoped crash `AnimRuntime`** (bound to the plane's own subtree so `healthy`/`destroyed`/`pieceN` are unique; resolved by NAME via `NameResolveFallback` because the def's ptrs are non-portable; a `Subset` program so only the crash's call-closure binds, not 800+ world defs) — the wreck breaks apart, the pieces tumble + scatter with inherited momentum, and the fireball/spark/smokeball/dirt/debris fire from the data. Verified end-to-end (C1 dive) + ambient world byte-identical. **Next: Wave 4 — flip `--data-crash` to default, retire the bespoke trio, close `docs/PLAN-M2-polish-4.md` item 8** (still ◐). Owed: the user's A/B playtest of `--data-crash` vs default across planes/chapters, and the `WreckMomentum` / `forward_rotation`-÷-run_time TUNE calls.
