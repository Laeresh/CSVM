...
- `src/Mech3/DestructibleRegistry.cs` — live mutable per-instance HP for `HEALTH>0` anim defs, one pool per `(def,anchor)`; feeds `ANIM_HEALTH` eval + the `DAMAGE_SEQUENCE` stages (`ApplyDamageStages`), inert until weapon fire decrements HP (C23).
...
- `src/Flight/Projectile.cs` — `ProjectilePool`: the shared-world weapon-fire subsystem — ballistics integration, tracers, muzzle flashes, per-surface impacts + sound; guns tracer-quad, rockets fly the FLYOUT MODEL body; `Spawn` into it.
