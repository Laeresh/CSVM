...
**Active plan: [`docs/PLAN-m3-polish-4.md`](docs/PLAN-m3-polish-4.md)** (10 items, waves A–D; written 2026-08-01). **Next:** `C9` — `BL-088`, Danger Zones score on the authored gate pair. Owed cockpit re-tests (`PT-13`–`PT-26`) remain in [`playtest.md`](playtest.md). Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
