...
**Active plan: [`docs/PLAN-m3-polish-3.md`](docs/PLAN-m3-polish-3.md)** (11 items, waves A–D, scaffolded 2026-07-31) — the 2026-07-31 cockpit-pass follow-ups: feedback that landed imperceptible, destructible behaviour, weapon-visual fixes, rockets. Wave position: A1 ☑ (`PT-13`), A3 ☑ (`PT-14`), A4 ☑, B11 ☑ (`PT-15`; B12's collider half fell with it). **Next:** C21, then C22/B12. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
