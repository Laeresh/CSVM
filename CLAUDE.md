...
**Four plan files sit in `docs/`. [`docs/PLAN-anim-debugger.md`](docs/PLAN-anim-debugger.md) is now banner-marked COMPLETE (Waves 1–5, landed 2026-07-23; archive to `docs/plans/` on the next cleanup)** — the permanent `--anim-lab` mode: the `PlaneViewer` split, additive runtime capabilities, the MVP, the def picker + authored-vs-fired timeline, and Wave 5's stage-placeless-defs-in-front-of-the-camera (which also delivered the crash plan's Wave 2a/2b scaffolding + the template-instancing `PlaceCalledTemplates`). **The active plan is now [`docs/PLAN-data-driven-crash.md`](docs/PLAN-data-driven-crash.md)** — its Layer 1 (generic `ObjectOpacityFromTo` + ballistic/scale/tumble `ObjectMotion` handlers) is developed and verified inside the lab (`--anim-lab --play-anim=player_crash_dirt`, where those events dispatch-but-inert today), then Layer 2 wires the flight crash to a **per-player** crash runtime (scoped `_index` so `healthy` is unique — the trap Wave 5 surfaced). `docs/PLAN-M2-polish-4.md` item 8 stays ◐ until that lands.
