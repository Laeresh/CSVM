...
**Active plan: [`docs/PLAN-animruntime-role-factories.md`](docs/PLAN-animruntime-role-factories.md)** (candidate 2 of the 2026-07-30 review) — two `AnimRuntime` role factories (`ForEffects`/`ForCrashRig`) + delete dead `Apply`. **Next:** B11 (`AnimRuntime.ForEffects` + rewire) — Wave A is done, unblocking Wave B. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
