...
**No active plan** — PLAN-sequencerunner-seam completed 2026-07-30 (indexed in [`docs/plans/plans.md`](docs/plans/plans.md)). Pick the next milestone/polish run from `backlog.md`, or scaffold one with `/new-plan`. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
