...
**Active plan: [`docs/PLAN-m3-polish-3.md`](docs/PLAN-m3-polish-3.md)** (11 items, waves A–D, scaffolded 2026-07-31) — the 2026-07-31 cockpit-pass follow-ups: feedback that landed imperceptible, destructible behaviour, weapon-visual fixes, rockets. Wave position: A1 ☑ (cockpit re-test `PT-13`), A3 ☑ (cockpit re-test `PT-14`). **Next:** the parallel-safe set C21/B11/A4. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
