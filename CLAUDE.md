...
**Original skydome rendered (2026-07-14):** `--fly` now shows the world's real `horizon` backdrop (zone1 day sky: sky2.tif haze dome + sea-haze skirt) instead of bare procedural sky — camera-anchored, 2.5×-scaled so depth-testing keeps it behind all terrain (see PlaneViewer bullet). Verified via scripted-flight screenshots: hazy horizon band at terrain height from low/high altitude, dome closed at zenith, no dome shadowing (before/after plane lighting identical), distant terrain not occluded. The procedural sky remains only as ambient-light source behind the dome.

Remaining known gaps: `--fly` renders horizon **zone1 (day)**, whose top cap is untextured flat gray in the source data (user-confirmed visible when climbing, 2026-07-14) — the original loads the C1 airfield **at night**, so the authentic sky there is **zone2**; switching is a polish item (needs the moon quad — moon1.png is RGB-on-black, likely additive in the original — and the `stars` mesh is empty in the extraction: engine-drawn or mech3ax gap, investigate). Sky UV scroll animation not implemented (`h_zone*scroll` — rate unknown). Clouds are alpha-scissor cutouts (original alpha-blends; they read as hard gray facets when flying at deck height). Prop/vehicle animations static, no propstart/propstop one-shots (mid-air spawn starts with engine running), no crash explosion visual (sound + hide only). With rendering, flight, sound, collision, and the skydome in place, **Milestone 2's core is essentially complete.** **Next step:** pick among the remaining polish items (candidates: zone2 night sky, crash explosion visual using the surveyed flame_ball.json emitters + fire flipbook textures, or alpha-blended clouds).
