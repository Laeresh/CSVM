...
- **Ground z-fighting — settled 2026-07-22 after FIVE wrong mechanisms; read `backlog.md`'s "Partition / ground-variant visibility" entry before touching it.** Two rulings, both from the user: **C1B is NOT a bug** (it is in the original — do not fix it, and **never raise `NodeOrderBias`**; the 30.95%→2.35% control was 1.4 points from erasing a faithful artifact). **C5 IS a defect but is not a depth bug**: the original runs a ground LOD chain (`cblock1_2`→`cblock1_1`→`cblock1`) and draws no `cblock[4-6]` in C5 IA1, while we draw `cblock4`+`cblock2`+`cblock1` coplanar in `g4683` — so we render variants the original never draws. **⛔ Do NOT raise `SurfaceRankBias`** despite it measuring 0.00%: it picks a winner among surfaces that should not be co-rendered (rules 4 and 8). Prime suspect is **partition visibility** (`WorldPartitionSetActive`). Evidence + committed instrument: `analysis/item9-depth-bias/`. C3's coast is **fixed** (`6c592c2`) — do not re-chase it.
