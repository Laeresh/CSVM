...
**No active plan.** `docs/PLAN-m3-polish-3.md` (11 items, waves A–D) completed 2026-08-01 and moved to [`docs/plans/`](docs/plans/plans.md). **Next:** pick up owed cockpit re-tests in [`playtest.md`](playtest.md) (`PT-13`–`PT-23`) or scope a new plan from `backlog.md`. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
