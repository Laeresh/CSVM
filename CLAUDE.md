...
Remaining known gaps: sky UV scroll animation not implemented (`h_zone*scroll` — rate unknown). Star/beacon dots use a fixed 3 px point size (source params 0.17/30/4000/6000 undecoded) and don't twinkle (the original's stars do). Clouds are alpha-scissor cutouts (original alpha-blends; they read as hard gray facets when flying at deck height). Prop/vehicle animations static, no propstart/propstop one-shots (mid-air spawn starts with engine running), no crash explosion visual (sound + hide only). With rendering, flight, sound, collision, and the night skydome in place, **Milestone 2's core is essentially complete.** **Next step:** pick among the remaining polish items (candidates: crash explosion visual using the surveyed flame_ball.json emitters + fire flipbook textures, or alpha-blended clouds).
