...
- If a task is delegated to a subagent use cheaper models when appropriate. For example sonnet for exploration.
...
| `--view=1-9` | hold a numpad flight-camera perspective for the run (2 belly, 4/6 flanks, 8 ahead); `--fly`/`--stunt` only. Layout settled; the magnitudes are a TUNE — [`docs/cli.md`](docs/cli.md) |
...
**Active plan: [`docs/PLAN-m3-polish-3.md`](docs/PLAN-m3-polish-3.md)** (11 items, waves A–D, scaffolded 2026-07-31) — the 2026-07-31 cockpit-pass follow-ups: feedback that landed imperceptible, destructible behaviour, weapon-visual fixes, rockets. Wave position: A1 ☑ (`PT-13`), A2 ☑ (`PT-18`), A3 ☑ (`PT-14`), A4 ☑, B11 ☑ (`PT-15`), B12 ☑ (`PT-16`), C21 ☑, C22 ☑ (disproven, fixed by C21), C23 ☑, D31 ☑ (`PT-17`). **Next:** D32. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
