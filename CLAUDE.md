...
**Active plan: [`docs/PLAN-m3-polish-2.md`](docs/PLAN-m3-polish-2.md)** (14 items, waves A–D, scaffolded 2026-07-30) — M3 combat look & feel from `backlog.md`. Wave position: waves A/B mostly landed (A3, B13 in flight); D31 in flight. **Next:** C21 (`BL-015`), then C22, D32, C24/C25. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
