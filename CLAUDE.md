...
**Four plan files sit in `docs/`. [`docs/PLAN-anim-debugger.md`](docs/PLAN-anim-debugger.md) is now banner-marked COMPLETE (Waves 1–5, landed 2026-07-23; archive to `docs/plans/` on the next cleanup)** — the permanent `--anim-lab` mode: the `PlaneViewer` split, additive runtime capabilities, the MVP, the def picker + authored-vs-fired timeline, and Wave 5's stage-placeless-defs-in-front-of-the-camera (which also delivered the crash plan's Wave 2a/2b scaffolding + the template-instancing `PlaceCalledTemplates`). **The active plan is now [`docs/PLAN-data-driven-crash.md`](docs/PLAN-data-driven-crash.md)** — its **Layer 1 landed 2026-07-23** (Wave 1): the generic `ObjectOpacityFromTo` → `OpacityFade` and ballistic/scale/tumble `ObjectMotion` → `MotionRuntime` handlers, verified byte-identical across 8 chapters bar C3's one ambient opacity fade, and firing in the lab (`--anim-lab --play-anim=player_crash_dirt`). **Next is Layer 2 (Wave 2/2c):** wire the flight crash to a **per-player** crash runtime (scoped `_index` so `healthy` is unique — the trap Wave 5 surfaced), building the real `PlaneBuilder.BuildDestroyed` piece geometry (whose gamez indices the lab's meshless anchors lacked, so the piece `ObjectMotion`s must resolve there — see the AnimRuntime bullet). `docs/PLAN-M2-polish-4.md` item 8 stays ◐ until that lands.
