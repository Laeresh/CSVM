...
- `docs/plans/` — **completed plans, archived** (each banner-marked `COMPLETE` with its date): the two M2 polish runs, M2.5, the mech3ax-fork revival, animation playback, the anim/rendering follow-ups and this docs cleanup — plus `upstream-pr/`, the shelved upstream-contribution package. Kept for their evidence, measurements and recorded dead ends; read them as history. The status table below lists each with its scope and date. **A plan sitting in `docs/` rather than in here is the active one** — that is the signal; right now there is none.
...
**No plan is active** — every plan file lives in `docs/plans/`, so the next plan written into `docs/` is by construction the single live document.
