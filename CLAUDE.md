...
**Two plans sit in `docs/`.** [`PLAN-M2-polish-3.md`](docs/PLAN-M2-polish-3.md) is **finished** — every one of its ten items is landed or closed-as-disproven; it stays out of `docs/plans/` only until its user-owned weather-zone A/B is handed over, then it moves with a `COMPLETE` banner. **The active plan is [`docs/PLAN-M3-weapons.md`](docs/PLAN-M3-weapons.md)** (written 2026-07-22) — Milestone 3, weapons and destruction: 44 items in six waves, scope settled with the user. **No item is user-gated** — both user-owned inputs landed 2026-07-22: the gun mount-name table (from which the slot→firepoint binding rule fell out — reverse index order, `W𝑛 → fp(9−2𝑛), fp(10−2𝑛)`), and the `CLUSTER_SIZE` playtest (stock Bloodhawk = 9 HE rockets, 3 per hardpoint → `CLUSTER_SIZE` is rounds-per-slot, `AMMO_LIMIT` a purchase cap). Wave A (research/docs) is the place to start.
