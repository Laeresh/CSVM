...
**No active plan.** The PlaneViewer split (Launcher / GameSession, [`docs/plans/PLAN-planeviewer-split.md`](docs/plans/PLAN-planeviewer-split.md)) completed 2026-07-30. Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
