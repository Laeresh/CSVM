...
/// them into one parameterised stop: every shipped bug in this family (`BL-224`, `BL-233`, `BL-236`,
/// `BL-242`) was a SELECTOR error, so the selectors are the distinctions worth naming at the call
/// site.</para>
///
...
public sealed class EmitterDirector
{
    ...
    /// (`BL-242`), all four defs whose stop missed start the emitter at their OWN template root and
    ...
    /// stop a stop-less PUFFER_STATE has (`BL-224`). `he_trails`' five spurt columns carry no
    /// ACTIVE_STATE 0; what the data turns off is the HOST, and until this was honoured a rocket's
    /// smoke ended only at the 32 s runtime TTL or when the next rocket restarted the def.
    ///
    ...
    /// tanks, `BL-236`). Emission ends rather than the emitter being torn down, so live particles
