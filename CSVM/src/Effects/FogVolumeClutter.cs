...
public sealed partial class FogVolumeClutter : Node3D
{
    ...
    // Camera-facing billboard, hand-rolled because a MultiMesh cannot use Godot's billboard flag.
    // The clutter fade's draw distance is scaled by the viewing angle against each sprite's own
    // polygon normal. A dropped quad collapses to a point under `cull`, so it costs no fragments. The
    // field is one static MultiMesh with no streaming.
