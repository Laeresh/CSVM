...
/// The source is named so a log line can say which layer the run is obeying.</summary>
public readonly record struct ResolutionPlan(int Width, int Height, string Word, string Source);

/// <summary><c>Words</c> is the sizes a resolution picker can offer for one screen, ascending.
/// <c>Fallback</c> is the word a saved size the list does not hold falls back to, the screen's own
/// size, which the list always holds. <c>ScreenWidth</c> and <c>ScreenHeight</c> are that screen's
/// own pixels, the ceiling <see cref="Including"/> keeps a size under. A list built with no screen
/// to ask carries a zero ceiling, which filters nothing.</summary>
public readonly record struct SizeList(IReadOnlyList<string> Words, string Fallback, int ScreenWidth, int ScreenHeight)
{
    ...
    /// sorts. That lets a size written into <c>options.json</c> by hand be shown, picked and saved
    /// back rather than replaced by a listed one. A word the list already holds, one that is not a
    /// size, and one the screen cannot hold each leave the list unchanged. What the picker offers
    /// is then still what the screen can hold.</summary>
...
/// <summary>
/// The window's size, and which display modes honour it, as sizes in
/// <see cref="OptionsStore.FormatResolution"/>'s spelling rather than a vocabulary. A picker's list
/// is built per screen by <see cref="Sizes"/> from <see cref="DisplayServer.ScreenGetSize"/>, and
/// widened by <see cref="SizeList.Including"/> with the size the options file names. The sources
/// layer as <see cref="MonitorSetting"/>'s do, the saved size then the screen's own, with the
/// display mode over both. Borderless pins the size to the screen, which <see cref="Pinned"/>
/// reports, and exclusive fullscreen takes it as a render size, Godot's window there owning its
/// own. Only <see cref="Apply"/> calls <see cref="DisplayServer.WindowSetSize"/>, after the mode.
/// ⚠ Godot exposes no video-mode list, so a picker offers the screen's size and the standard sizes
/// inside it.
/// </summary>
public static class ResolutionSetting
{
    ...
    // The sizes a picker offers where the screen holds them, ascending. They cover the shapes a
    // desktop monitor is sold in: 4:3, 16:9, 16:10, 21:9 and 32:9. The 4:3 ladder carries four
    // rungs rather than one because the original's own frame is 4:3. That is the shape a player
    // most wants a choice within. A ceiling filter over this is the whole of the per-monitor list,
    // Godot having no mode enumeration to ask. A size the table lacks reaches the picker through
    // the options file (SizeList.Including).
    ...
    /// with no screen to ask can offer. A shell composed without an engine (an engine-free test)
    /// is the only such caller. A run at the controls goes through <see cref="Sizes"/> and gets
    /// the screen's own list.</summary>
    ...
    /// borderless default. A file naming no mode therefore pins the size too.</summary>
    public static bool Pinned(string? displayModeWord) =>
        DisplayModeSetting.Resolve(displayModeWord).Word == DisplayWords.Borderless;

    /// <summary>The size the three sources resolve to is the screen's own where
    /// <paramref name="displayModeWord"/> pins it, else <paramref name="savedWord"/> where
    /// <paramref name="offered"/> holds it or <see cref="SizeList.Including"/> would. Otherwise
    /// the list's fallback. ⚠ A saved size the screen cannot hold falls back to the screen's size,
    /// never to the nearest offered one. A distance over sizes has no right answer.
    /// docs/architecture/Utils.md.</summary>
    ...
    /// at that size inside the screen-sized window Godot gives it, through <see cref="Render"/>. A
    /// borderless one is left alone at the screen's own size, which its mode already gave it.</summary>
    ...
    // What a run renders at inside a window whose size it does not own. Godot's exclusive
    // fullscreen on Windows neither switches the display mode nor takes a WindowSetSize, the
    // window staying at the screen's size through both. A chosen size can therefore only be the
    // render target, drawn at that size and scaled up with its shape kept. Zero, and a size the
    // window already stands at, put back the project's own one-to-one presentation.
    ...
    // A resize grows from the window's top-left corner. A size larger than the one standing would
    // push the bottom-right off the screen, and take the plaques the player just pressed with it.
    // Centred on the screen the window already stands on, which is the screen a monitor setting
    // moved it to before this ran.
