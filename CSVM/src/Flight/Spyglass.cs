...
/// <summary>The spyglass's decoded rules, engine-free: the disc's own geometry, the fog-derived
/// range gate with its engage/release asymmetry, the field of view that holds a target at a
/// constant apparent size, and the camera pose at the pilot's own aircraft. The picture is
/// <see cref="SpyglassView"/>'s, where it sits on the pane is <see cref="EdgeMarker"/>'s, and what
/// is drawn around it is <see cref="TargetHud"/>'s; this module answers only how far, how wide and
/// which way. Decode: <see href="../../docs/org/spyglass.md">org/spyglass.md</see>.</summary>
public static class Spyglass
{
    /// <summary>The authored <c>sgwin</c> window's side, 96 x 96 in every chapter's gamez, read at
    /// the 1440p reference and scaled by <see cref="HudMetrics"/> like every other HUD metric.
    /// </summary>
