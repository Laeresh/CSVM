...
public sealed partial class ProjectilePool : Node3D
{
    ...
    /// struck collider, it carves the bowl and flattens the decorations in it, and answers whether
    /// the carve landed. Asked only for a collider whose node carries
    /// <see cref="SceneBuilder.CanModifyMeta"/>, which no shipped node does. Wired to
    /// <c>CraterField.TryCarve</c> in a collidable flight; null in a build with no world colliders.
    /// </summary>
        ...
        Godot.Collections.Array<Rid>? owner = null)
    {
        ...
        // The terrain carve runs between the hook and the row's bindings, under the hook's Effects
        // bit. ⚠ Do not drop the CanModify test. No shipped node carries the flag, so the test is
