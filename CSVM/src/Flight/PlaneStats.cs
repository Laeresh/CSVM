...
public sealed class PlaneStats
{
    ...
    // feed B2's per-frame slot update, DistFactor B4's candidate scoring; Inaccuracy is B5's and
    // lands with it. Fallbacks are the executable's own compiled defaults, not the shipped
    // player.json values — DistFactor's shipped 0.0 deletes the scan's distance term outright,
    // where the compiled fallback below does not.
