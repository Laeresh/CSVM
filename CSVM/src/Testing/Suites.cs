...
public static class Suites
{
    ...
    // The targeting HUD on AI hostiles, in two halves. The pure selection (VersusHud.NearestHostile
    // over a constructed candidate set, no scene) pins the filters and HostileTag; the in-engine half
    // runs the matchless tracker against real spawned AI planes in a live pool, covering acquisition,
    // the switch to a closer hostile, the crash drop and the empty-pool null. A hud built without a
