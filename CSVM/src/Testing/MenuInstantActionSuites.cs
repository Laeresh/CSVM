...
internal static class MenuInstantActionSuites
{
    ...
    {
        var box = Row(shell, OriginalShell.LivesKey);
        ...
        ctx.Check(!shell.Rows.Any(r => r.Key != OriginalShell.LivesKey && r.Width > 0f && r.Height > 0f
        ...
        Press(host, seat, Down);
        ctx.Check(shell.FocusedKey == OriginalShell.LivesKey, $"the walk reaches it from the wingman count ({shell.FocusedKey})");
        Press(host, seat, Right);
        ctx.Check(ia.Lives == 2 && Row(shell, OriginalShell.LivesKey)?.Label == "2",
            $"a sideways step writes the shared feature's lives ({ia.Lives})");
        Press(host, seat, Left);
        Press(host, seat, Left);
        ctx.Check(ia.Lives == 0 && Row(shell, OriginalShell.LivesKey)?.Label == "Unlimited",
            $"and zero reads Unlimited, the word the Built-in stepper writes ({Row(shell, OriginalShell.LivesKey)?.Label})");
        Press(host, seat, Accept);
        ctx.Check(shell.OpenDropdown == OriginalShell.LivesKey && shell.Rows.Count == InstantActionFeature.MaxLives + 1
        ...
        ctx.Check(shell.OpenDropdown == null && ia.Lives == 3 && Row(shell, OriginalShell.LivesKey)?.Label == "3",
            $"a click on a row picks that count and closes the list ({ia.Lives})");
        ctx.Check(ia.IsAceDuel && Row(shell, OriginalShell.LivesKey) is { Enabled: true }
            && Row(shell, "IA_D_NENEMY0") is { Enabled: false },
            $"the ace duel leaves it live where it blanks the enemy boxes ({Row(shell, OriginalShell.LivesKey)?.Enabled})");

        var contents = Row(shell, OriginalShell.ContentsKey + ":1");
        if (contents != null)
        {
            Click(host, seat, Pointer(fit, contents.X + 5f, contents.Y + 5f, pressed: true, clicked: true));
            ctx.Check(ia.PresetIndex == 1 && ia.Lives == 3 && Row(shell, OriginalShell.LivesKey)?.Label == "3",
        ...
        ctx.Check(shell.PoseInstantActionLives(1) && ia.Lives == 1 && Row(shell, OriginalShell.LivesKey)?.Label == "1",
