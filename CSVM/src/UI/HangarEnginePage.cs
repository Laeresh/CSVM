...
/// displacements, then the same three with nitro) plus the explicit no-engine row (langui 1171),
/// the chosen one ticked. The ←→ stepper makes the focused row the scratch plane's engine and
/// Confirm advances without editing, the airframe page's idiom. The detail line is the engine's
/// decoded cost and weight through <see cref="HangarEconomy.EngineLine"/>, plus the original's
/// power stat (the stat-table rating times the decoded per-id factor, display-only).
/// </summary>
public sealed class HangarEnginePage : HangarPage
{
    ...
    public override string RowText(int row) =>
        Flow.EngineName(Scratch.Airframe, row) + (Scratch.Engine == row ? "  ✓" : string.Empty);
