...
/// click as a press edge and the wheel's steps since the last poll. The device reads are
...
public sealed class PointerSeat : IMenuInputSource
{
    ...
    /// <paramref name="pressed"/> and the wheel's steps since the last read from
    /// <paramref name="wheel"/> (positive toward a list's foot; a seat with no wheel omits it).</summary>
    public PointerSeat(IMenuInputSource inner, Func<(float X, float Y)?> position, Func<bool> pressed, Func<int>? wheel = null)
    {
    ...
    public MenuCommands Poll(float dt)
    {
        ...
        _wasPressed = pressed;
        return frame with { Pointer = new MenuPointer(at.X, at.Y, pressed, clicked, wheel) };
    }
