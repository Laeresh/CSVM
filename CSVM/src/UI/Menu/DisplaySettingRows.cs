...
/// the forgiving reads the resolvers already make, restated where a row needs an index rather than
/// a plan. The screens and the sizes are enumerated per machine by
...
public static class DisplaySettingRows
{
    ...
    /// <summary>Where a saved word sits among a row's own values, and every word row's default: a
    /// word the vocabulary does not know, or none saved at all, shows as the first value, which each
    /// display vocabulary orders as the behaviour with no options file.</summary>
    public static int WordIndex(IReadOnlyList<string> words, string? word)
    {
    ...
    /// own sizes rather than a vocabulary, so the first of them is the smallest the monitor holds
    /// and not the behaviour with no options file; a size this screen does not offer shows as the
    /// project default, which every list holds. That is
    ...
    public static int ResolutionIndex(IReadOnlyList<string> words, string? saved)
    {
        ...
        int fallback = IndexOf(words, CSVM.Utils.ResolutionSetting.Default);
