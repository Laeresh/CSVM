...
- `BL-536` `[Bug]` **A periodic 95 to 170 ms stall about every 190 frames, attributed to no sample site.** Seen in every `--fly --chapter=C1 --plane=player_bhawk --crash=300 --no-vsync --perf` run, before and after the crash-rig pre-warm: `HitchMonitor` records `samples=none` with `gc0_delta=1`, `gc1_delta=1` and `physics_ms` at 50 to 100 ms on the same frame, on a roughly fixed cadence. Not the effects runtime (nothing in `PerfSample` claims any of it) and not the first-use construction the pre-warm removed. The 79.91 ms `samples=[]` trip in the DamageLab controls capture has the same shape.
  *Where to look:* what runs on a ~190-frame cadence (a gen1 collection is on the record, so what allocates ~17 MB per period; the collision hull work saw the same 130 to 180 ms gen1 frame at frame 300 to 305 of a 310-frame run), and whether `physics_ms` is the cause or a victim of the same pause. ⚠ PERF-13: compare only within one vsync mode.
