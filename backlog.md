...
- `BL-879` `[Bug]` `[S]` `[Next: code]` `[Impact: low]` `[Evidence: trace]` **The Original Controls page's scrollbar slides with the list: on a tab longer than its window (Other), the arrows and the thumb move up and down together instead of the thumb alone.** *Evidence:* seen at the controls on the Other tab. `OriginalControls.cs:778` (`ComposeKeysBar`) and `:602` (`KeysLists`) both put the bar's top at `page.LineY(0, _keysTop)`, and `LineY` (`:845`) is `ListY + (row - top + 1) * ItemHeight`, so the bar's top is the *scrolled-away* first row's line and falls by one row per line scrolled; the rows themselves are placed at `LineY(i, _keysTop)` and stay in the window. Any tab over `KeysWindowRows` (12) shows it; Other is the one that is. *Fix shape:* anchor the bar at the window's first line, `LineY(_keysTop, _keysTop)` or `ListY + ItemHeight`, at both sites, and pin it in the controls suite over a scrolled Other tab (bar top and arrow positions unchanged between line 0 and line 3). *Cross-refs:* `BL-842` (the thumb's fixed height, same bar), `CSVM/src/Testing/MenuOriginalControlsSuites.cs`.

- `BL-880` `[Fidelity]` `[S]` `[Next: code]` `[Impact: low]` `[Evidence: decoded]` **The Plane Construction hub's CURRENT WEIGHT previews an airframe row by judging the current build's weight on the candidate and reddening it; the original's airframe swap shows "CURRENT WEIGHT: Pending" and is never red.** *Evidence:* user at the controls: the weight when selecting a new plane should not be compared against the plane it was built on. `OriginalHangar.cs:1717` (`HubBill`) prices the row under the cursor through `HangarFeature.BillWithAirframe` (`HangarFeature.cs:918`), a copy of the scratch plane with the airframe swapped and every other pick kept, and `HangarEconomy.cs:197` reddens the line when that sum exceeds the candidate's capacity. The decode (BL-754's closing commit, `docs/org/hangar.md`) has the set-airframe callback at `0x0040d2a9` write the dword at `0x00647ba4`, which swaps the weight line for langui 1032's "CURRENT WEIGHT: Pending" and makes callback 2214 answer true whatever the weight. The user's decision is (a): an airframe row's preview shows the Pending line and no comparison. *Fix shape:* when the open list is the airframe list, the hub's weight line takes langui 1032 and the plain ink; the cost line keeps its preview. Pin the `plane-construction:open` pose. *⚠ Traps:* do not touch the engine, armour, gun or hardpoint previews, which the original does compare (2214 runs on every other change). *Cross-refs:* `BL-754`/`BL-760` (the preview and the red), `BL-864`/`BL-865` (the swap's three-answer question).

- `BL-881` `[Bug]` `[S]` `[Next: code]` `[Impact: high]` `[Evidence: trace]` **The pause sheet's parchment marks a row by its briefing priority where the graph wants the objective number, so CM05 shows its first objective checked from two seconds in.** *Evidence:* seen at the controls on CM05. `GameSession.cs:3504` passes `objective.Priority` to `ObjectiveGraph.CompletedOf` (`ObjectiveGraph.cs:313`), whose parameter is the 1-based `OBJECTIVEn` number. `BriefingObjective` (`BriefingObjectives.cs:9`) carries the priority and the message key, not the number. On `C3/M04` the parchment's rows are `OBJECTIVE15` (priority 1) and `OBJECTIVE14` (priority 2), while `OBJECTIVE1` is a conditionless `BEGIN_DORMANT [2.0]` wake objective that completes on its first eligible tick, so `CompletedOf(1)` is true after two seconds. Every mission whose priorities are not its objective numbers is affected; one whose priorities happen to match shows nothing, which is why the pin passed. The load sheet reads the same rows. *Fix shape:* carry the objective's number on `BriefingObjective` (the reader already walks each `OBJECTIVEn` block) and mark on it; pin CM05 in `pause-sheet` (row 1 unmarked with `OBJECTIVE1` complete and `OBJECTIVE15` open, marked once 15 completes). *⚠ Traps:* the `--menu=pauseboard[:CM[:done]]` aid marks the first `done` rows by count and cannot show this; the briefing page numbers nothing itself, since the strings carry their own numbering. *Cross-refs:* `BL-802` (the Built-in readout's mark, which keys `def.Number`), `docs/formats/objectives.md` (the completion rule), `CSVM/src/Testing/PauseSheetSuites.cs`.

- `BL-882` `[Bug]` `[M]` `[Next: code]` `[Impact: high]` `[Evidence: trace]` **The Original pause sheet draws neither the ownship icon nor the zeppelin icon over a live campaign mission.** *Evidence:* seen at the controls on CM05, both absent. `GameSession.cs:3507` to `:3529` build both: the ownship from the paused seat's rig (`RigOf`, its controller's global position and forward) and the zeppelin from the first `piratezep` node the anim runtime finds, each handed to `PauseScreens.AddWorldIcons` (`PauseScreens.cs:229`) and placed through `MissionMap.Icon` and `EscapeMap.TryProject` (`EscapeDialog.cs:46`), which draws nothing for a position outside the chart's world window. Three candidate causes, none yet excluded: the shared block's `OWNSHIP`/`MYZEP` bitmap read empty, so the branch never runs; the world-to-chart conversion (world X across, negated world Z down, each `WORLD` bound truncated toward zero) placing a real position off the window, which the user's own read favours; or the rig lookup answering null at pause time. *Fix shape:* a suite or a shot, not a live-only probe: compose the readout over a headless CM05 world with the player at `PLAYER_INIT` and assert two placed icons inside the clip; if the projection is at fault, cross-check it against `docs/org/pause-screen.md`'s worked example (CM01's `TRAVELERS` waypoints inside the island). *⚠ Traps:* the `--menu=pauseboard` door has no world and draws no icons by design, so it cannot reproduce this. *Cross-refs:* `BL-821` (the sheet), `BL-881` (the same readout's marks), `docs/org/pause-screen.md` (the projection rule and its constants).
