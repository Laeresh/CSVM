...
- `BL-331` `[Feature]` `[M]` `[Next: code]` `[Impact: high]` `[Evidence: decoded]` **The ground shadow is placed and shaped like the original's, but it lies on a flat quad rather than conforming to the ground**
  ...
  own triangles rasterised top down into the modulate texture every frame off the node the
...
- `BL-894` `[Bug]` `[S]` `[Next: data]` `[Impact: low]` `[Evidence: feel]` **The gyrocopter's shadow carries a still top rotor where the original's rotor shadow turns.** *Evidence:* the user's recall of the original at the controls: the rotor's shadow rotates. `GroundShadowPass.cs:149` reads an aircraft's shape once, on first sight, through `GroundShadowSilhouette.For(model, ...)` (`GroundShadowSilhouette.cs:92`), so a part that turns relative to the airframe keeps its first-sight pose in the raster; `PropAnimator` (`PropAnimator.cs:9`) spins the blur discs on the model only. `docs/org/shadows.md` says the original rasterises the model node per frame, which is what carries a turning rotor into its shadow. *Fix shape:* confirm on footage (`Z:\CSVM\OriginalScreenshots\Videos\`) or in the decode that the rotor's shadow turns, then re-rasterise the turning part each frame at the disc's current turn, or compose the rotor as a second turning silhouette over the still airframe. Whether the fixed-wing propeller discs turn in their shadows too is the same question. *⚠ Traps:* a per-frame raster of the whole model for every aircraft is the pass's own cost, and that raster is quartic in the texture's edge, not square (`docs/org/shadows.md`'s last section has the measured numbers); raster only the turning part. *Cross-refs:* `CSVM/src/Flight/PropAnimator.cs`, `docs/org/shadows.md`.
