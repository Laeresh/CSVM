...
- `BL-857` `[Feature]` `[L]` `[Next: code]` `[Impact: high]` `[Evidence: decoded]` **The spyglass: Shift+S toggles a round live
  picture of the selected target at its off-screen marker, and the marker's arrow and text sit
  differently from ours.** *Evidence:* decoded in full into
  [`docs/org/spyglass.md`](docs/org/spyglass.md). The spyglass and the edge marker are ONE object,
  `DAT_0071d284`, driven by one function `FUN_0049d940`, so the arrow, the "N o'clock" tag and the
  picture stop and start together. Shift+S (action `0x30`, `LAB_00489260`) flips a byte that is
  `1` at level load and is refused only while the player's plane is dead (`+0x91d`); every other
  gate is re-read per frame. The picture appears only while the target is OFF SCREEN and inside
  `fogNear + (fogFar - fogNear) * 0.8` capped at 2000, engaging at 0.875 of that (the fog range is
  the live per-zone one, `FUN_004da920` on `world1`); no target class is tested anywhere. The
  window is the `sgwin` gamez node, 96 x 96 device pixels in every chapter, its origin MOVED each
  frame to the marker and masked by a circle of radius `ftol((96 + 1) * 0.5)` = 48. Camera 2 stands
  at the player's aircraft and aims at the target, field of view `2 * atan(1.1 * R / d)` clamped to
  `[1.5 deg, 90 deg]` with `1.1 * R` kept as a high-water mark, and its roll is zeroed for anything
  but an aircraft. The marker anchor is the projected point clamped into the viewport inset by 5%
  per axis plus half the window; the arrow is a SHAFT from that anchor (pushed out to the disc's
  rim when the disc is up) to the point clamped to the FULL viewport, plus a head 20 px long and
  10 px across (`0x006035e4`, `0x006036bc`); the label anchor keeps the anchor's x and offsets y by
  `+3` in the screen's upper half, `-45` in the lower (`0x006035d0`, `0x006082d4`), or
  `discBottom + 3` / `discTop - 45` when the disc is up. *Fix shape:* three separable pieces.
  (a) `Flight/TargetHud.cs` geometry: `RefArrowLen` 18 to 20 and `RefArrowHalf` 8 to 5, add the
  shaft `DrawArrow` never draws, and replace the `RefArrowLen + RefTextGap` back-off along the
  arrow direction with the decoded vertical `+3` / `-45` rule. (b) `Flight/EdgeMarker.cs`: the flat
  `RefEdgeMargin = 46f` becomes 5% of each pane axis, and `Resolve` gains the second clamp
  rectangle (the full pane) that the arrow's tip needs, since one call now yields two points.
  (c) the picture itself: a `SubViewport` per human with the decoded pose and field of view, a
  round mask 96 reference px across, toggled by a new `InputAction`. *⚠ Traps:* `EdgeMarker` stays
  engine-free, so the second clamp is another `Placement` field, not a camera reference. Do NOT
  port the occluder pass (`FUN_004c8f70` walk): the retail build records each class-5 node's field
  and flag and writes the same values back, a no-op. Do NOT port the slot-9 branch (60 degrees, 15
  units ahead): every `Target` vtable carries the constant-zero stub there. *Cross-refs:*
...
- `BL-882` `[Bug]` `[M]` `[Next: code]` `[Impact: high]` `[Evidence: trace]` **The Original pause sheet draws neither the ownship icon nor the zeppelin icon over a live campaign mission.** *Evidence:* seen at the controls on CM05, both absent. `GameSession.cs:3507` to `:3529` build both: the ownship from the paused seat's rig (`RigOf`, its controller's global position and forward) and the zeppelin from the first `piratezep` node the anim runtime finds, each handed to `PauseScreens.AddWorldIcons` (`PauseScreens.cs:229`) and placed through `MissionMap.Icon` and `EscapeMap.TryProject` (`EscapeDialog.cs:46`), which draws nothing for a position outside the chart's world window. Three candidate causes, none yet excluded: the shared block's `OWNSHIP`/`MYZEP` bitmap read empty, so the branch never runs; the world-to-chart conversion (world X across, negated world Z down, each `WORLD` bound truncated toward zero) placing a real position off the window, which the user's own read favours; or the rig lookup answering null at pause time. *Fix shape:* a suite or a shot, not a live-only probe: compose the readout over a headless CM05 world with the player at `PLAYER_INIT` and assert two placed icons inside the clip; if the projection is at fault, cross-check it against `docs/org/pause-screen.md`'s worked example (CM01's `TRAVELERS` waypoints inside the island). *⚠ Traps:* the `--menu=pauseboard` door has no world and draws no icons by design, so it cannot reproduce this. *Cross-refs:* `BL-821` (the sheet), `docs/org/pause-screen.md` (the projection rule and its constants), `docs/verification.md` WORLD-40 (the readout's two numberings).
...
- `BL-886` `[Feature]` `[S]` `[Next: code]` `[Impact: high]` `[Evidence: decoded]` **An Instant Action sortie on the Original presentation pauses on the Built-in board; the original pauses on the Instant Action blackboard.** *Evidence:* `OriginalScreenshots/Instant Action Pause Menu.png` (a stunt run): the load screen's blackboard with the three photographs down the left, "INSTANT ACTION" and the mission type as the two heads, the type's description and its win line on the board, the four strips Resume, Restart, Preferences and Quit along the foot and the wings emblem, no map, no memento, no parchment. `GameSession.cs` `BuildOriginalPauseBoard` returns null unless a campaign mission is running, so the sortie falls back to the Built-in board, though the reader is already there: `EscapeDialog.InstantActionFile` (`ia_escape.json`) and its `loading_i<env><letter>` key, and `PauseSheet.Load`'s `instantAction` flag, which every caller passes false. The decode is `docs/org/pause-screen.md` "An Instant Action pause draws the blackboard, not a map": `ia_escape.zrd` opens for an Instant Action session (`FUN_004a0d20` under `FUN_004639b0`), its dialogs carry `loadframempt2`, `MP-shotdown`, `MP-crash` and `mp-dangerzone2`, the four texts `HEAD1`, `HEAD2`, `OBJ5` and `OBJ6` and no `PRIMITIVES` block, and the strips come from the shared block; a multiplayer pause takes the same dialog. *Fix shape:* build the Original pause board for an Instant Action session keyed on the sortie's environment and mission-type letter, compose the blackboard dialog through the drawing `LoadScreens` already has for the same dialogs, carry the four strips and the wings, no objectives, memento or icons; pin the six environments in `pause-sheet` and one composed sheet against the still. *⚠ Traps:* the still's photographs are the load screen's own for that type, not mission stills; Restart must restart the sortie, not a campaign mission. *Cross-refs:* `BL-821` (the campaign sheet), `BL-884` (Preferences over the pause, the same strip), `docs/org/pause-screen.md`, `CSVM/src/UI/LoadScreens.cs`.
