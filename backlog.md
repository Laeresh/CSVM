...
## Instance the city-block collision shapes (C5 flight load +3.5 s)

**Raised by the user 2026-07-22, immediately after polish-3 item 6 landed.** The feature is
correct; this is purely its load cost. C5 `--fly` went **3895 ms → ~7400 ms**, and the delta is
entirely `ClutterBuilder.BuildSolidCollision`.

**Rendering is already instanced and is not the problem.** `BuildSolidInstance` emits one
`MultiMeshInstance3D` per kind over `SceneBuilder.SharedMesh(kind.MeshIndex)`, so 79,306 buildings
cost ~158 extra draw calls and 2–3 ms of GPU.

**Collision is fully expanded.** `BuildSolidCollision` walks every instance, transforms every
vertex of that kind's mesh into world space, and concatenates the result into one
`ConcavePolygonShape3D` per 1024 m region. Measured in C5:

| | |
|---|---|
| distinct building models | **32** (`cb00a`–`cb24a`, plus `cb00/01/02/03/06/08/11det01`) |
| placements | **79,306** |
| collision triangles built | **2,554,455** |
| distinct triangles among them | **~1,000** |

`cb14a` is placed 14,079 times and `cb12a` 12,397 — each re-transformed vertex by vertex. We build
roughly **2,500× more collision geometry than the scene actually contains**, then hand the physics
server a multi-hundred-thousand-triangle BVH per region to index it.

**The current code's reason for expanding is real but addressable.** Its comment argues that ~80k
`StaticBody3D` nodes would swamp the broadphase and the scene tree — true, and one whole-map
trimesh is worse. But those are not the only options: **`Shape3D` resources are shareable, and a
shape can be added to a body with its own transform.** Two ways up, in increasing order of payoff:

1. **Shared shape per model.** Build one shape per distinct `MeshIndex` (32 of them), then attach
   it many times. `PhysicsServer3D.BodyAddShape(bodyRid, shapeRid, transform)` adds a shared shape
   to one body *without* a scene-tree node per instance — which is what avoids the node explosion
   the current comment worries about. Keep the per-region body split for broadphase. Shape-build
   cost drops from 2.55M triangles to ~1,000.
2. **Convex or box shapes instead of concave.** A city block is box-like at aircraft scale. A
   `BoxShape3D` (or convex hull) per distinct model is ~free to build, much cheaper in both
   broadphase and narrowphase than a concave trimesh, and arguably *more* correct for gameplay —
   a plane should not slip through a window gap. Concave detail is only worth paying for if
   something at aircraft scale needs it, which nothing here does.

**Verify carefully — collision changes are exactly where this project's instruments lie.**
Re-measure load *and* frame time (the 60 fps vsync cap hides both headroom and cost —
`docs/verification.md` §2), and re-run the scripted C5 dive that currently logs
`CRASH into clutter/clutter_bld_-10_-4 (wing)`; the body name is the proof the new collider is the
one being hit. Confirm sprites stay pass-through (item 5) and that buildings do not become
*more* permeable if the shape kind changes. Cold-cache swings of 2–3× in both directions were
observed while measuring item 6, so average several runs.

**Related, already logged above:** 40% of those triangles are the `cbNNdet01` meshes, deliberately
not dropped because they are complementary halves of one building rather than a LOD pair. If the
shapes get instanced, that 40% stops mattering and the question can be dropped entirely.

**Also still open from item 6:** map-edge extension buildings render but are not collidable,
because merging a region trimesh at a boundary crossing would hitch. **Instanced shapes would very
likely remove that objection too** — attaching 32 shared shapes to a new region body is cheap
enough to do at a boundary crossing, so this and that item should be picked up together.
