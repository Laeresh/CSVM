...
- `BL-891` `[Bug]` `[S]` `[Next: code]` `[Impact: low]` `[Evidence: trace]` **A failed mission on a profile that has finished the campaign plays the closing cinema.** *Evidence:* user at the controls: failing a mission on a profile with every mission complete started the end film. `ClosingCinema.OpenScrapbook` (`ClosingCinema.cs:39`) gates on `CampaignProgression.Complete(profile)` alone, and `CampaignFlow.OpenScrapbookAfterMission` (`CampaignFlow.cs:567`) reaches it after every flown mission whatever its result, so a replay that fails, or any replay at all once the campaign is complete, plays `Final.MPG` again on a fresh `Launcher` (the `Played` latch is per instance, `ClosingCinema.cs:26`). *Fix shape:* play the film only when the mission just flown was won and its result is what completed the campaign (the profile's completion answer went from false to true on this result); a replay on a complete profile opens the scrapbook with no film. Pin in `campaign-loop` with a complete profile failing a replay and with a profile completing its last mission. A decode of the original's `callback 3104` (`docs/formats/cinemas.md`) would say whether the original gates on the result the same way and is optional. *Cross-refs:* `CSVM/src/Session/ClosingCinema.cs`, `CSVM/src/UI/CampaignFlow.cs`.
