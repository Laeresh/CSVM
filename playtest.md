...
|---|---|---|---|
| `CAP-02` | **A level cliff run flown twice — once wings-level, once deliberately banked** | The effect is found and it is **lateral**: 17° of heading swing at 34 °/s against under 1° in two matched free-air controls (`BL-095`). One thing now stands between that and a result — **heading cannot tell a yaw from a banked turn**, and the pipeline has no roll readout for a daylight scene (`moon.py` needs the moon). So fly `Run4 Clip3` again, unchanged, but **hold the wings level through the pull and say so**; then fly a third pass where you *deliberately* bank ~45° into a turn at the same speed, as the counter-example. If the wings-level pass still swings the heading and the banked one looks visibly different in the same instruments, yaw is established. Two cheap additions that would settle it outright: keep a **landmark on the horizon** in frame so roll is visible directly, or fly it in a **night mission** so `moon.py` gives roll for free. Cockpit view, **auto head turn OFF**, no damage, stay under γ ≈ +45° so the compass never goes degenerate | `BL-095` |
