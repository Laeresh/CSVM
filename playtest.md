...
| ID | Capture | What must be in frame | Unblocks |
|---|---|---|---|
| `CAP-46` | The Pandora's broadsides against the player's aircraft | In the original, fly C3/M03 into the Pandora's broadside arc (abeam the hull, inside `cannon_fire_range`) and hold there through a hatch deploy and at least one volley, external/chase view. The controls report says the original's broadsides engage zeppelins only; the decode says `targets [player]` resolves and fires on the aircraft (`docs/formats/mission-entities.md` "Broadside firing"). *Look for:* whether the hatches open with only the aircraft in arc, and whether rounds come at the aircraft | `BL-567` |
