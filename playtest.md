...
|---|---|---|---|
| `CAP-02` | **Re-fly the ORIGINAL mountain scenario**, shallow — the one shot that never got measured | Fourteen takes found no amplification on either axis (`BL-095`): matched held-pull loops at 1,113 ft and 90 ft give identical `a_n` plateaus, and ADI body pitch rate is 0.91–1.10× free air. One anomaly survives — `Up Down` recovery #1 at **2.30×** — and it is the least trustworthy reading in the set (30% gated at γ ≈ −63°). Everything built to reproduce it came back flat, but all of those were over **water**; the sideways flick was first reported over a **mountain**, and that geometry has never been cleanly measured because those takes are near-vertical over unknown terrain height. So: fly the batch-1 scenario again — same mission, aimed at the same rising terrain — but **shallow, under ~45° of dive**, so nothing gates and the compass stays meaningful. If it comes back flat too, ground blow is not observable in flight state and `BL-095` should stop asking. Cockpit view, **auto head turn OFF**, no damage | `BL-095` |
