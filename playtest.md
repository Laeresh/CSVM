...
|---|---|---|---|
| `CAP-02` | A **level run straight at a cliff**, held into the flick | The free-air control (`Run3 5`) **retracted** the magnitude claim: near the ground the pull reaches only **0.91×** the aircraft's own free-air maximum, so there is no excess load factor to measure (`BL-095`). What is left is the pilot's account that pitch feels boosted — and `a_n` cannot test it, because `a_n` is how sharply the *flight path* bends while a **body pitch-rate** boost leaves it untouched. So fly **level and straight at a cliff or mountain face** and hold it into the flick. Constant altitude with only range closing is what separates a range trigger from a height one in the horizontal; and being level and shallow, this is the first `CAP-02` geometry where the **ADI and the compass are both meaningful** — the ADI gives body pitch rate (`adi.py`), the compass gives the lateral escape. Both are degenerate in a vertical dive, which is why no take so far answers either. Cockpit view, **auto head turn OFF**, no damage, γ under ~64° | `BL-095` |
