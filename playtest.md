...
|---|---|---|---|
| `CAP-02` | The same cliff run **at night**, or with a horizon landmark in frame | The effect is found, it is **lateral**, and a commanded turn is excluded: the wings-level cliff pull steps 17° onto a new heading and holds it, while deliberate 45° banks turn 3× faster at peak, 10× faster sustained, and keep turning (`BL-095`). One gap remains — a **small transient roll (~15°) would produce the same step**, and the pipeline still has no roll readout for a daylight scene. Two ways to close it, either sufficient: fly the identical cliff run **in a night mission**, where `moon.py` recovers roll for free from the moon; or fly it in daylight with a **distant horizon landmark held in frame** so image-plane rotation is directly visible. If roll stays flat through the heading step, ground blow is a **yaw** and `BL-095` can close on it. Cockpit view, **auto head turn OFF**, no damage, stay under γ ≈ +45° so the compass never goes degenerate | `BL-095` |
