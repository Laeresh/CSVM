...
|---|---|---|---|
| `CAP-02` | A **free-air** max pull, and a **level run at a cliff** | Mechanism and both constants are measured (`BL-095`): an elevator-channel assist that doubles pitch authority inside ~400 units of along-path range to terrain. Two gaps left. (1) **The ×2 rests on a borrowed yardstick** — neither clip holds a hard free-air pull, so it is compared against `BL-109`'s loop at a different speed (lift ∝ V²). Fly a **max-elevator pull at ~330 mph above 2,000 ft**, nothing within range, same aircraft, ideally same session. (2) **Range was only tested downwards.** One shallow over-water dive separated range from altitude; the claim that it is *distance to whatever you are aimed at* wants the horizontal case — a **level run straight at a cliff or mountain face**, cockpit view, where altitude is constant and only range closes. That is also the shot that would show whether the escape goes lateral, which is the one thing about the sideways flick still unmeasured (heading is degenerate in a vertical dive, so no existing take can give it). Both: cockpit view, **auto head turn OFF**, no damage, γ under ~64° | `BL-095` |
