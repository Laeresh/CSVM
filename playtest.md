...
| `CAP-28` | Torpedo flight dynamics | ⚠ **No longer needed to answer `BL-290` — the decay is decoded** ([`docs/org/ordnanceTypes.md`](docs/org/ordnanceTypes.md)): the round inherits the launcher's velocity and blends it out linearly over `LOCK_ON`, 2.5 s for `wep_14`. A measurement cannot beat the constant, so do not film this to contest it. Worth filming only as a post-implementation A/B, and even then the decode is the reference | `BL-290` (answered) |
