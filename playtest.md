...
| ID | Capture | What must be in frame | Unblocks |
|---|---|---|---|
| `CAP-39` | Cockpit view while firing | In the original, hold the guns for a second or two IN the cockpit view (mode 6), ideally at dusk or against a dark cliff so a light flash reads. A sweep of all 130 existing clips found no cockpit-view firing footage: every gun clip is nose or chase view. *Look for:* whether the muzzle flash lights the instrument panel at all, or only the world and wings — CSVM excludes the interior from the per-shot effect light on the render-pass argument (`docs/org/cameraViews.md`, the mode-6 interior draw), which this clip confirms or refutes; the windshield bullet-hole decals on taking window hits ride the same sortie if one happens | `BL-436` (the interior-immunity call), `BL-431` |
