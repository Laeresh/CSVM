...
| `CAP-35` | C2 fog vs camera altitude (the H1/H2 discriminator) | C2 IA1: pick one ground feature in the LA basin and hold it in view through a steady climb 500 → 5000 ft, then level off above ~3,400 ft and pan ground ↔ horizon. HUD altimeter in frame throughout. What it decides: whether the distant-ground fog wash **disappears once the camera climbs above ~3,360 ft / 1024 m** (fog fades by *camera* altitude) or **stays on the ground regardless of your altitude** (fog fades by *fragment* altitude — our current shader). C2 is the only chapter whose flown fog band (256–1024 m) is inside the flight envelope, so no other flight can answer this | `PLAN-overcast-match` `B13`/`B14` |
