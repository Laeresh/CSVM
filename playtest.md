...
| `CAP-36` | C5 night street haze: entry, dwell, exit | Any C5 mission, at night, flown **low between the skyscrapers** — one continuous take: level approach toward a street, a descent **into** the ground haze, at least 5–10 s of **dwell inside it** at street height, then a climb back **out** above it. What must be readable: (a) does the picture **darken/close in over the last few metres of approach** and how abruptly (we ramp it over the authored 16 m, so it is roughly one frame at speed); (b) what the view looks like **while inside** — how far you can see down the street, and whether the interior look is a curtain that clears after the first ~16 m or a steady near-blackout; (c) whether the **exit** is symmetric with the entry. External/chase is fine; a HUD altimeter helps but is not required. ⚠ Any capture of C5 at street level is useful even if the flight is not this exact shape — nothing on record films C5 low at all. Verifies `C21` (the in-volume whiteout, landed 2026-08-09 against the decompiled ramps: approach 16 m, interior decay 16 m, colour `[16,16,16]` — near black, not white) and `C22` (`ZONE3`'s 50–250 m fog inside). ⚠ Nothing in the remake is tuned against a guess here and nothing may be: the ramps and the colour are decompiled constants, so this capture **confirms or refutes** them rather than calibrating them | `PLAN-weather-decompile-match` `C21`/`C22` |
### Damage & collision
