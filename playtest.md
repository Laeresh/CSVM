...
| ID | Capture | What must be in frame | Unblocks |
|---|---|---|---|
| `CAP-40` | Plane Construction screen with visible prices | Open the original's Plane Construction screen and hold it long enough to read every price on screen, plane by plane and armor option by armor option, plus the layout around them (list, panel, selected-plane readout). Only five static `Campaign *.png` shots exist today and none show this screen. ⚠ This capture is for the on-screen numbers and layout, never a footage-derived measurement (`docs/verification.md`): read the prices as printed, don't time or scale anything off the clip | `A5`, `B13` |
| `CAP-41` | The Previous Missions list | Open the original's Previous Missions list from the cabin screen and hold it long enough to read its layout: what each entry shows (mission name, result, any icon or status), ordering, and how a completed vs. not-yet-reached mission differs on the list. ⚠ Layout and on-screen labels only, no timings read off the clip | `C22` |
...
| `CAP-44` | Cabin screen ambience/idle behaviour | Sit on the original's cabin screen without touching any menu option, held long enough to catch what it does on its own: any idle animation, ambient sound loop, background motion, or lack of one. Pair with a pass over each menu option's hover/selected state if the same sortie allows. ⚠ Qualitative only: what is present and what it looks/sounds like, never a duration read off the clip | `C22`, `E42` |
