...
| `bl-058-clutter-doubling/` | Does C5 draw doubled clutter buildings now that the subface fix hides the base ground layer? | Yes — `Clutter.cs` matches templates by texture only, blind to the `Subface` flag; both `cblock1/2/3`'s district (43,873 placements) and `cblock4/5/6`'s disjoint district (35,433) stamp in the same build. Strong candidate fix (exclude the always-buried `cblock4/5/6` district, its ground texture never wins the z-fight and correlates r≈0.70 with its paired subface as the same scene at two fidelities) — held pending `playtest.md` `CAP-22`. See `FINDINGS.md`, tracked as `BL-250`. |
