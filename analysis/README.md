...
| `campaign-coop-4p-perf/` | What is the frame cost and hitch behaviour of four panes over the heaviest shipped campaign mission (`PLAN-campaign-coop.md` D33)? | `CM18` (C4/M03) at 1P/4P x external/cockpit: draws grow 5.7x from 1P to 4P (faster than the 4x pane count) while render/GPU ms stay inside noise; cockpit view adds ~5% draws at either player count. The mission's one dominant frame-time event, a ~300 ms `ai_spawn` hitch at two fixed sim frames, is identical at 1P and 4P, so it is not a splitscreen cost (filed as `BL-641`; the still-open per-viewport question stays on `BL-434`). See `FINDINGS.md`. |
