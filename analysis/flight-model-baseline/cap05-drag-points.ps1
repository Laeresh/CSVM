...
.DESCRIPTION
    ...
    drag(x) = maxThrustAccel * x^DragExpLow, a power law those four points had been FITTED to. It is
    now the original's own drag polar, which is a parabola in MACH:

        q     = 0.5 * rho * V_fps^2                   lb/ft^2, dense band
        M     = V / (1109.5 * 0.3048)
        C_D   = 0.73 * (0.12 + 0.8*M + 0.5*M^2)
    ...
    a fit. The curve carries no lift/AOA/load-factor term at all - the original passes its lift
    coefficient to the drag routine and never reads it - so this is a pure function of speed and the
    condition of the flight (level, banked, pulling) does not enter.
...
Write-Output "x      Mach    model(m/s^2)  measured(m/s^2)  err"
foreach ($p in $points) {
    $v     = $p.X * $fdSpeed
    $q     = 0.5 * $rho * [math]::Pow($v * $feetPerMetre, 2)
    $qS    = $q * $refArea
    $mach  = $v / $soundMs
    $cd    = $polarScale * ($polarPara + $polarLin * $mach + $polarQuad * $mach * $mach)
    ...
    $line  = [string]::Format($inv, "{0,-6:0.00} {1,6:0.0000} {2,12:0.0000}  {3,15:0.00}  {4:+0.0;-0.0}%", `
        [double]$p.X, [double]$mach, [double]$model, [double]$p.Measured, [double]$err)
    Write-Output $line
}
