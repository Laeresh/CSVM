...
async function runPowerShellHook(cwd: string, event: any): Promise<HookResult> {
  ...
  const psScript = `
    ...
    if (-not $c) { exit 0 }; 
    $q = [char]39; 
    if ($j.tool_name -eq 'Bash' -and ($c -match ('@' + $q + '\\\\r?\\\\n')) -and ($c -match ('(^|\\\\n)' + $q + '@'))) { 
      [Console]::Error.WriteLine('BLOCKED: PowerShell here-string (@' + $q + ' ... ' + $q + '@) in the Bash tool, which takes a heredoc. Bash treats the @ lines as literal text instead of failing, so the content lands corrupted (this has silently mangled commit messages). Use a heredoc: cmd <<' + $q + 'EOF' + $q + ' ... EOF. For a multi-line commit message prefer Write to a file, then: git commit -F <file>.'); 
      exit 2 
    }; 
    if ($j.tool_name -eq 'PowerShell' -and (($c -match ('<<-?\\\\s*' + $q + '?[A-Za-z_])) -or $c.Contains('/dev/null'))) { 
      [Console]::Error.WriteLine('BLOCKED: bash syntax (heredoc or /dev/null) in the PowerShell tool. Use a here-string @' + $q + ' ... ' + $q + '@ with the closing delimiter at column 0, and $null instead of /dev/null.'); 
      exit 2 
    }; 
    exit 0
  `;
...
async function runPowerShellBashGuard(cwd: string, event: any): Promise<HookResult> {
  ...
  const psScript = `
    ...
    $c = $j.tool_input.command; 
    if (-not $c) { exit 0 }; 
    $segments = $c -split '&&|\\\\|\\\\||;|\\\\|' | ForEach-Object { $_.Trim() } | Where-Object { $_ }; 
    $other = $segments | Where-Object { $_ -notmatch '^(git|gh)\\\\b' }; 
    if (-not $other) { exit 0 }; 
    [Console]::Error.WriteLine('Use Powershell instead of bash'); 
    exit 2
  `;
...
async function runPowerShellDotnetFormat(cwd: string, event: any): Promise<HookResult> {
  ...
  const psScript = `
    $j = [Console]::In.ReadToEnd() | ConvertFrom-Json; 
    if ($j.tool_input.command -match 'RunTests\\\\.ps1|dotnet\\\\s+test|git\\\\s+commit') { 
      dotnet format 'CSVM/CSVM.csproj' --verbosity quiet; 
      $buildOutput = dotnet build 'CSVM/CSVM.csproj' --verbosity quiet --nologo -t:Rebuild 2>&1; 
...
async function runPowerShellDuplicateIdCheck(cwd: string, event: any): Promise<HookResult> {
  ...
  const psScript = `
    $j = [Console]::In.ReadToEnd() | ConvertFrom-Json; 
    if ($j.tool_input.command -notmatch 'git\\\\s+commit') { exit 0 }; 
    $root = git rev-parse --show-toplevel 2>\\$null; 
    if (-not $root) { exit 0 }; 
    $defs = @(); 
    $b = Join-Path $root 'backlog.md'; 
    if (Test-Path $b) { 
      $defs += @(Select-String -Path $b -Pattern '^\\\\s*-\\\\s+\\\\`(BL-\\\\d+)\\\\`' -AllMatches | ForEach-Object { $_.Matches } | ForEach-Object { $_.Groups[1].Value }) 
    }; 
    $p = Join-Path $root 'playtest.md'; 
    if (Test-Path $p) { 
      $defs += @(Select-String -Path $p -Pattern '^\\\\s*(?:-\\\\s+|\\\\|\\\\s*)\\\\`((?:PT|CAP)-\\\\d+)\\\\`' -AllMatches | ForEach-Object { $_.Matches } | ForEach-Object { $_.Groups[1].Value }) 
    }; 
...
async function runPowerShellEncodingCheck(cwd: string, event: any): Promise<HookResult> {
  ...
  const psScript = `
    ...
    $rx = [regex]('[' + [char]0xC2 + [char]0xC3 + [char]0xE2 + '][' + $follow + ']'); 
    $bad = @(); 
    foreach ($f in ($changed | Sort-Object -Unique)) { 
      if ($f -notmatch '\\\\.(cs|md|json|ps1|gdshader|gdshaderinc|txt|csproj|sln|tscn|tres|cfg|godot|yml|yaml)$') { continue }; 
      $p = Join-Path $root $f; 
      if (-not (Test-Path -LiteralPath $p -PathType Leaf)) { continue }; 
      try { $t = [IO.File]::ReadAllText($p) } catch { continue }; 
      $m = $rx.Match($t); 
      if ($m.Success) { 
        $line = ($t.Substring(0, $m.Index) -split [string][char]10).Count; 
        $bad += ($f + ':' + $line) 
      } 
    }; 
    if ($bad) { 
      $bad | ForEach-Object { [Console]::Error.WriteLine('Mojibake (UTF-8 read as ANSI and re-saved) about to be committed: ' + $_) }; 
      [Console]::Error.WriteLine('Cause: a PowerShell file write without UTF-8 - a Get-Content/Set-Content round-trip without -Encoding utf8, or a BOM-less file read as ANSI. Repair the corrupted characters before committing; edit repo text with the Read/Edit/Write tools, and pass -Encoding utf8 on both read and write whenever PowerShell must touch a repo text file.'); 
      exit 2 
    }; 
    exit 0
  `;
...
async function runPowerShellGoldenExercisesCheck(cwd: string, event: any): Promise<HookResult> {
  ...
  const psScript = `
    ...
    $root = git rev-parse --show-toplevel 2>\\$null; 
    if (-not $root) { exit 0 }; 
    $p = Join-Path $root 'analysis/goldens/manifest.json'; 
    if (-not (Test-Path -LiteralPath $p -PathType Leaf)) { exit 0 }; 
    try { $m = [IO.File]::ReadAllText($p) | ConvertFrom-Json } catch { [Console]::Error.WriteLine('analysis/goldens/manifest.json does not parse as JSON - fix it before committing.'); exit 2 }; 
    $bad = @(); 
    foreach ($s in $m.shots) { 
      $e = [string]$s.exercises; 
      if ($e.Length -gt 250) { $bad += ($s.name + ': exercises is ' + $e.Length + ' chars, cap is 250') }; 
      if ($e -match '(BL|PT|CAP)-\\\\d+|PLAN-|[Aa]lso exercises|\\\\d{4}-\\\\d{2}-\\\\d{2}') { 
        $bad += ($s.name + ': exercises carries history or an item id -- ' + $Matches[0]) 
      } 
    }; 
    if ($bad) { 
      $bad | ForEach-Object { [Console]::Error.WriteLine('BLOCKED: ' + $_) }; 
      [Console]::Error.WriteLine('analysis/goldens/manifest.json: exercises describes what a shot covers TODAY - one sentence, under 250 chars, no item ids, no dates, no past-change deltas. REWRITE the field on a re-pin, never append; the history belongs in the commit message and git log -p. See analysis/goldens/README.md.'); 
      exit 2 
    }; 
    exit 0
  `;
