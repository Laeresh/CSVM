...
/// <c>Rect2.HasPoint</c>'s inclusive-at-position / exclusive-at-end asymmetry, which is today's
/// behaviour and not a bug to fix here.
/// </summary>
public class EdgeMarkerTests
{
    private const float Margin = 46f;
    ...
    {
        var placed = EdgeMarker.Resolve(new Vector2(300f, 500f), behind: false, Pane, Margin);
    ...
    [Fact]
    public void TheMarginBoundaryIsInclusiveAtTopLeftAndExclusiveAtBottomRight()
    {
        Assert.True(EdgeMarker.Resolve(new Vector2(Margin, Margin), false, Pane, Margin).OnScreen);
        Assert.False(EdgeMarker.Resolve(new Vector2(Pane.X - Margin, 400f), false, Pane, Margin)
            .OnScreen);
        Assert.False(EdgeMarker.Resolve(new Vector2(500f, Pane.Y - Margin), false, Pane, Margin)
            .OnScreen);
    }
    ...
    public void BehindForcesOffScreenAndMirrorsTheDirectionThroughCentre()
    {
        ...
        var placed = EdgeMarker.Resolve(new Vector2(700f, 400f), behind: true, Pane, Margin);
        Assert.False(placed.OnScreen);
        Assert.Equal(new Vector2(-1f, 0f), placed.Dir);
        Assert.Equal(new Vector2(Margin, 400f), placed.Anchor);
    }
    ...
    public void AProjectionOnCentreFallsBackToPointingDown()
    {
        ...
        Assert.Equal(new Vector2(Pane.X / 2f, Pane.Y - Margin), placed.Anchor);
    }

    [Fact]
    public void TheAnchorLandsOnTheInsetBoundaryAlongTheDirection()
    {
        // Cardinal: far off the right edge clamps to x = width − margin at the same height.
        var right = EdgeMarker.Resolve(new Vector2(5000f, 400f), false, Pane, Margin);
        Assert.Equal(new Vector2(Pane.X - Margin, 400f), right.Anchor);
        ...
        var corner = EdgeMarker.Resolve(new Vector2(1200f, 1200f), false, pane, 40f);
