...
every run and therefore has no repo home to be copied from. One row is in the table without being
in the zip at all, `LICENSE-plmpeg`, because it is an input the notices file is assembled from;
its Zip path column names it as no zip row and it takes no `$ReleaseFiles` entry.
...
| `LICENSE-thirdparty.txt` | `packaging/LICENSE-thirdparty.txt` | The notices the two binaries' contents oblige the zip to carry: the Godot engine, its own third-party components, the self-contained .NET runtime, and the Rust crates in `unzbd.exe`, plus one notice for source rather than payload, `pl_mpeg`, which CSVM's managed MPEG-1 decoder is ported from. Assembled by `packaging/BuildThirdPartyNotices.ps1` from the shipped artefacts themselves; its header names the three payload versions it speaks for and the export refuses to ship it against any other |
