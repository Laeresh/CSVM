...
- **Landing each item** — flip its checklist status ☐/◐ → ☑ (or ❌ if disproven), rewrite its per-item section to lead with **Landed.** / **Verified.** and keep the pre-landing text under **Original approach (kept for reference).**, then commit. The commit ritual (a message body carrying the what-landed/verification record, PROJECT_CONTEXT.md "Current status" refresh, `Co-Authored-By` trailer naming the acting agent, main only) is the **`commit-next`** skill — use it per item.
