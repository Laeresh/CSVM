...
| **The energy test reads the ARRIVING speed: the body's own velocity taken into world, plus the velocity the frame it is solved in is travelling at** | The solve is in the parent frame, so a piece riding a wreck that is still descending at about 104 m/s arrives carrying none of that in its own velocity, and the local reading rests it the instant it touches while the frame goes on carrying it under. On C2B/M04's Gemini that put `gasbag1` and `gasbag4` at −13.1 m and −26.9 m under the sea the other three floated on. The original writes its landing once and stops, so it has the same shape; here the piece re-meets the surface on each frame it is carried through it, which is the cell query's own "a body already under a surface is lifted back onto it" |
