...
| Divergence | Why |
|---|---|
| **The column tier is a pair of rays, down first and up only when the down ray answers nothing**, not a grid-cell record pick | The engine has no terrain-grid cell database to query, so the column is drawn as a ray. Two of them are what makes the answer independent of the body's height, as the cell query's is: the down ray gives the nearest surface below and the up ray the nearest above, which is the original's own nearest-to-`y` pick wherever a cell holds one ground surface. ⚠ A down ray alone goes blind the moment a body is under the surface, so a step that overshoots can never be recovered and the body drifts to its watchdog |
...
| **The per-step admission test and its `COMPLEX` widening decide as much here as they do in the original** | Both engines can answer with a surface *above* the body, so a climbing step admitted by `COMPLEX` can be stopped by a ceiling. A downward-only column would leave both inert, which is why the second ray and this test belong together |
