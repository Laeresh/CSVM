...
- **The column tier** (`FUN_004e9e30`, the default) queries a **terrain-grid column** at the body's
  ...
  bytes each, height at `+0x14`. ⚠ The caller saves the flying node's `intersect_surface` bit
  (`FUN_004ccf50` at `004e97d2`), clears it (`FUN_004ccf00` at `004e97da`), queries (`004e97fa`)
  and restores it (`004e9809`), and the node walk `FUN_004c7b60` returns before a node's children
  when that bit is clear, so the body's **whole subtree** is invisible to its own column. A
  zeppelin section carries its own structure 20 m below its origin, and a column that answered
  with it would land the section on itself.
...
| **The column's self-guard is an exclusion list**, the colliders of the body's own subtree gathered once at launch and handed to both rays | Godot has no per-query flag bit to clear, so the subtree the original's cleared `intersect_surface` hides is excluded by collider instead. A section's colliders are built with the world and never added to, so the launch-time set stays complete |
...
| **The per-step admission test and its `COMPLEX` widening decide as much here as they do in the original** | Both engines can answer with a surface *above* the body, so a climbing step admitted by `COMPLEX` can be stopped by a ceiling. A column that stopped at the body would leave both inert, which is why the second ray and this test belong together |
...
| **The energy test reads the ARRIVING speed: the body's own velocity taken into world, plus the velocity the frame it is solved in is travelling at** | The solve is in the parent frame, so a piece riding a frame that is still moving arrives carrying none of that in its own velocity, and the local reading rests it the instant it touches while the frame goes on carrying it under. The original writes its landing once and stops, so it has the same shape; here the piece re-meets the surface on each frame it is carried through it, which is the cell query's own "a body already under a surface is lifted back onto it" |
| **A contact the body did not approach under its own power charges the WATCHDOG instead of a contact** | How many frames a descending wreck takes to settle is a frame-rate figure, so spending the contact cap on it makes the resting height depend on the frame rate; the 15 s backstop bounds a frame that never stops instead |
