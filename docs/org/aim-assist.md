...
| Who gets it | the local player only (`param_1 == DAT_0071c298`); every other plane takes the AI dead-eye branch | built (B6, 2026-08-13) — `FlightController.IsHumanPiloted` (default true). CSVM's splitscreen has no single "local player"; the condition that generalises is human-versus-AI, not pane 1 (Decision 7), so every human pane is assisted and CSVM has no AI planes yet to gate out |
