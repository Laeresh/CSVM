...
**Of the 27, three are in the census and out of this plan:** Audio, ControlsPrefs and Keys. No
shared option exists behind any of them, so Original draws their two page doors disabled; a later
plan that adds a shared audio or controls option owns the page it belongs on.
GameOptions and Video are built: the difficulty and the presentation stand on the first, the
graphics mode, the display mode and the V-Sync choice on the second, and `PF_B_GAMEOPTIONS` and
`PF_B_VIDEO` are live.
...
⚠ **Built-in has no counterpart for GameOptions, Video, Audio, ControlsPrefs, Keys or
