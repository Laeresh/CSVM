...
6. ☑ Splitscreen join + plane select — Start-to-join in the launchscreen, simultaneous pick, 2P/4P layouts, per-player HUD *(done 2026-07-19; `src/UI/MenuInput.cs` + `src/Flight/HudMetrics.cs` new, LaunchMenu rebuilt around a player list. Join on Start (P1 = keyboard + every unclaimed pad — a set, not `pads[0]`, so the phantom-device fix survives), join strip on every screen, P1 drives Mode/Chapter, all players pick planes simultaneously with coloured cursors + lock, B unlocks/leaves. Per-player planes through `PlaneFor(i)` + a per-plane stats cache; `--plane=a,b` is the CLI equivalent. HUD scale = window height damped by sqrt(pane share) — the identity in 1P, so the regression is provable: 1P menu pixel-identical, static orbit byte-identical, 1P fly/stunt differ in 4/16 sampled px (0 in HUD bands) vs a 13,346-px noise floor. Same-turn fixes: bottom-anchored gauge dials, scoreboard fits a pane, throttle on its own line below full screen, content-aware menu layout scale. `--debug-join=N` screenshots the multi-cursor screen. **Open: the menu→multi-player launch is unverified at runtime (needs two controllers) — join/un-join, lock race, per-pad binding, Esc-from-splitscreen owed a user playtest; pane HUD sizes TUNE)*
