...
7. ☑ Splitscreen stunt race — per-player progress/timers, shared end scoreboard *(done 2026-07-19; `src/Flight/StuntRace.cs` + `src/Flight/StuntRaceBoard.cs` new. Every player races an independent copy of the zone list (`StuntMission.ForAnotherPlayer` — parsed once per session), with their own marker HUD, clock and progress; finishing parks that pilot at the finish showing their placing + a "waiting for N pilots…" line while the field flies on, and the last finisher raises a full-window ranked board (placing / colour-coded tag / aircraft / zones / total + gap to winner, winner in their identity colour). R = rematch (restarts every player), gated until everyone is in. Verified: 2P/4P boards ranked correctly with four different aircraft; the decisive mid-race shot shows P1 finished 1st while P2 still flies with its own 0:00.9 clock, 0/5 zones and its own marker; a scripted 2P straight-line run 60 m apart completed dz2 for P1 only (0/5 vs 1/5 in the panes) proving progress is per-player; an auto-rematch soak looped 5 races with byte-identical totals each time (no state leak) and no errors. Solo path unchanged (splits board + `ScoreStore` best time preserved at 2:02.8); free-flight 1P/2P, orbit and a no-dzone chapter at 2P all clean. Open: user playtest with two controllers — board fonts/placement, whether a finished pilot should keep flying rather than freeze, rematch feel)*
