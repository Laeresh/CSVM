...
analysis findings, or git history; module constraints belong
...
- **GOLD-6** — **A pure TIMING change moves pixels, and a particle shot amplifies it without
  ...
  timing (the retired `analysis/bl-135-callsequence-lag/`,
  `git show analysis-archive:analysis/bl-135-callsequence-lag/FINDINGS.md`): moving every called sequence's first
...
- **LOG-13** — **Do not overlap engine probes.** The one exception is `RunTests.ps1`'s own engine
  ...
  bit-identical at every count (`PLAN-fast-verification` C21). What still does not
...
- **INSTR-30** — **A world-coordinate precision effect cannot be measured at heading 0, nor with a
  ...
  the geometry. Measured on the cockpit panel (`PLAN-cockpit-panel` C20): the pinned heading-0 nudge read 0.03-0.12
