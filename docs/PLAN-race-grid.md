...
| 3 | How does the countdown flight work? | **On rails** — kinematic level walk handing off at `_model.Reset(...)`, so GO *is* today's spawn; no sink, no per-plane divergence, decoupled from `BL-074` |
...
53.6 m/s goes stale when **`BL-074`** makes spawn speed plane-dependent, and on a mixed-aircraft
grid a speed-derived offset lands each plane differently. This plan avoids the exposure entirely by
not simulating anything at spawn; the deferred countdown item must not reintroduce it.
