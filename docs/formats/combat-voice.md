...
The clip inventory and its naming are surveyed in `docs/SCOPING-M4-ai.md` § 6; this page covers the
...
- **`DA` is the ally counterpart of `DE`, and the split is by team, not by outcome.** Both are the
  ...
  if it is not. `docs/SCOPING-M4-ai.md`'s open question 6 recorded this as inference — it is now
...
- The **clip inventory** — 2,520 files, 31 pilot ids, the 11 `TYPE` families and their counts — is
  `docs/SCOPING-M4-ai.md` § 6. This page covers only what the engine does with them.
