...
def; properties resolve **nearest-first** through the chain (e.g. `pbloodhawk` â†’
`player_airplane` â†’ `basic_airplane`). Player planes are the defs whose chain contains
...
| `engine` | engines.json row id â†’ power factor |
| `engine_sound` / `cockpit_engine_sound` | sound-def names (SETS in sounds.json) â€” only `cockpit_engine_sound` unconsumed (needs a cockpit view) |
| `damaged_engine_sound` | `[[soundName, f0, f1]]` â€” one shared `basic_airplane` entry (`snd_damagedengine`, 0.0, 1.0) covers every plane; `f0`/`f1` are undecoded and read as a fade window over accumulated damage fraction (below) |
| `dynamics` | nested dict: `pitch_torque`, `roll_torque`, `rudder_torque`, `return_rate`, `ang_momentum_damp`, `rec_moments_inertia` (xyz), `fd_speed` (m/s), `drag_factor`, `veh_weight`, `ref_area`. The parser also accepts `level_off_rate`, which **no shipped def authors** â€” see below |
...
| `bullethole_anims`, `weapons`, `turrets`, `cannon_jam`, `armor`/`health`, AI tuning | only `turrets` consumed (M4 C9a) â€” [Weapons, damage & AI keys](#weapons-damage-and-ai-keys) below |

## Units, dynamics, and engines

Units are meters/seconds: `fd_speed` 135 m/s â‰ˆ 302 mph matches the Bloodhawk's published
top speed; `flight_ceiling` 2500 m. `player.json` holds player-global values â€”
`nom_gravity` = 20 m/sÂ² (an arcade 2 g) â€” plus the sound curve blocks
([sounds.md](sounds.md)).

The `dynamics` block: `rec_moments_inertia` is the *reciprocal* inertia per axis
(x = pitch, y = yaw, z = roll); steady-state rotation rate = torque Â· recInertia /
`ang_momentum_damp` (Bloodhawk roll â‰ˆ 1.65 rad/s). `return_rate` is extra centering
applied when the stick is released.

âš  `fd_speed` was documented here as "the full-throttle level-speed equilibrium". **Treat that as
...
decoded drag polar backwards from each airframe's `fd_speed` also fails to close â€” the Balmoral
...
**The 13th `dynamics` field: `ThrustFactor` is not authored â€” it is engine power.** The shipped
...
`power / (drag_factor Â· C_D)` reproduces their `fd_speed` order exactly (Spearman +1.000), which a
uniform thrust factor does not. `level_off_rate` is the mirror case â€” a token the parser accepts
that no def uses.

`engines.json` is a flat list of rows `[id, name, power]`; a plane def's `engine`
property picks its stock engine by id. âš  **Every player airframe's stock engine is its Lvl-2 row**
(ids 11, 14, 17, 20, 23, 26, 29, 32, 35, 38, 41 â€” Bloodhawk: 11 = Lvl-2, power 0.62), never Lvl-1;
solving a constant from a Lvl-1 row inflates it by ~30 %. `power` is the plane's `ThrustFactor`,
and the original applies it as `Thrust = power Â· ref_area Â· thrustAvailable(Mach) Â· throttle` â€”
...
[name, hp, hp, flagsâ€¦,
 ...
 "injure_anims", [[fraction, animName, rootName], â€¦]]
```

- `name`: `nose` / `tail` / `leftwing` / `rightwing` for every player plane.
- **The pair is (hit points, armor)** â€” `[1]` is the zone's hit points, `[2]` its **armor pool**,
  spent first. Settled against the original's armory; see [below](#armor-and-hit-points).
- **The two values are equal in every entry** â€” all 88 parts across the 22 defs that carry
  ...
  `PlaneDamage.Apply(part, healthDamage, armorDamage)` â€” armour first, 1:1 overflow â€” landed
   (`PLAN-armour-layer`).
  this install has an unequal pair.)*
- Flags: `critical` â€” the plane is destroyed when this part reaches 0 HP (all four player
  parts carry it); `engine` â€” engine damage/power loss on that part.
  âš  **The `critical` reading is from the flag's name and the  executable decode does not
  ...
  `pfirebrand`/`pavenger`/`pkestrel` â€” it
...
- `injure_anims`: **descending HP fractions**; when the part's HP fraction crosses one,
  ...
  - `<part>_damage_green` (0.72) / `_yellow` (0.46) / `_red` (0.20) â€” the cockpit
    ...
    **combined** progression of both pools: yellow = up to half the zone's armor gone, orange =
    ...
    `backlog.md`). âš  **A reading, not a decode** â€” `_damage_yellow` at 0.46 sits mid-band rather
    than on 0.375, so the correspondence is suggestive and does not pin the mapping down.
    Source: Crimson Skies PC manual, damage-indicator section â€”
    <https://manualmachine.com/gamespc/crimsonskies/1119420-user-manual/>.
  - `<part>_damage_effects` (0.99) â€” **not part of that cycle, despite the neighbouring
    ...
    `pdp2`, **followed by an unconditional third call at `pdp4`** â€” so a hit sparks one panel or
    ...
    `snd_ricochet1â€“4` sequences. âš  **Only `player_pfighter` ships the 0.99 entries** â€” measured
    ...
    âš  **`random_gun_impact`'s real home is `weapons.json`, not here â€” read this entry as a probable
    authoring leftover (hypothesis, ).** It is the `player` **IMPACT surface animation**
    for `wep_03` (60slug) â€” "what a bullet does when it hits the player's aircraft"
    ...
    feature â€” but no capture of the original settles it, so it is a reading, not a finding, and the
  ...
  - `pdpanelN` â€” flips the exterior torn-skin panel `pdpN` (planes.zbd nodes; the anims
...
`small_fireball_follow`, `short_firetrail` / `loop_short_firetrail` â€” the discrete-puff
...
`_h` node** â€” yet `player_destruct_reset.json` (`plane_reset`) re-ACTIVEs
...
`pdpN`â†”`pdpN_h` numbering is crossed on three plane models â€” pair tornâ†”healthy by mesh
...
data can separate (armor, hp) from (hp, hp) or (max, current) â€” the reading stood as a hypothesis
...
2. **The armory's per-zone cap is 60 units**, uniform across a plane's four zones. (Whether the cap
   varies by airframe is untested â€” one airframe was read.)
3. **A stripped zone falls far faster** than an armored one â€” greenâ†’red in visibly less time, more
   ...
   figure â€” and does not need to. With armor equal to hp at stock and armor spent first with 1:1
   overflow, the **2Ã— effective pool is entailed by the model**, not a separate quantity to measure.

`CAP-19` is discharged and retired from [`playtest.md`](../../playtest.md).

âš  **The zrdr number is the *stock* allocation, not a fixed property of the airframe.** A player
buys more. Every pair being equal is a fact about stock loadouts, **not** a licence to fold armor
into hp â€” see `BL-085`.

Corroborating evidence, all data-confirmed:

1. **The HUD showed two pools.** `messages.json` `MSG_HUD_HEALTH` = `Armor: %1%% Health: %2%%` â€”
...
2. **Weapons carry both damage figures, and they differ.** Every one of the 46
   ...
   `HEALTH_DAMAGE`, and 18 of them differ â€” the ammo matrix is built out of the split:
...
4. **Retail shipped a per-zone armor purchase.** `rof/ui_strings.json` id 1039 `IDS_AR_TITLE`
   = "3) ADD ARMOR", ids 1044â€“1047 = Nose / Tail / Left Wing / Right Wing â€” exactly the
   ...
   "the **default** armor, engine, and guns for this new airframe" â€” a stock allocation exists.
...
5. **Retail states armour-first depletion outright** â€” `ui_strings.json` id 3372 (AP: "hardened
...
**Still open: what `ARMOR: Standard (N/T/W)` is â€” and it is now known *not* to be the cap.** Five of
...
Autogyro 300/300/200). It is **not** the stock allocation â€” stock is ~20 â€” and `CAP-19` has now ruled
...
Three measured constraints on what it can be: retail-triple Ã· zrdr-part-sum is
13.75 / 16.7 / 12.0 / 21.7 / 16.7 across the five, so **no linear map** relates them; triple Ã· 60 is
ragged for the same reason; and at the armory's observed 4 lbs/unit a **fully** armored airframe is
4 Ã— 60 Ã— 4 = **960 lbs** against a `veh_weight` of 1900 â€” a real trade-off, where the
...
weight model can carry. The armory's own constants â€” per-unit cost and weight, per-zone caps â€” are
...
following `prop1`: a black-smoke trail (COLORS ramp: born orange 255,164,90 â†’ near-black
5,5,5) plus a fire trail, both `DISTANCE_INTERVAL` emitters (one puff per N meters of the
node's motion â€” 1.0 m smoke / 0.25 m fire). `player_fuelleak` runs a `fuel_trail` at a
random pdp panel (unwired).

## Collision probes

Each player def carries a `collision` list of six xyz points in the plane's local frame
(nose âˆ’Z, right +X, meters) â€” the original's own collision representation, apparently
probe points: nose, right wing, left wing, tail, top, belly. pbloodhawk:

```
(0, 0, âˆ’5.68)  nose        (0, âˆ’0.20, 4.55)  tail
(+5.71, âˆ’0.82, +1.87) right wing   (0, +0.80, âˆ’1.46) top (canopy)
(âˆ’5.71, âˆ’0.82, âˆ’1.87) left wing    (0, âˆ’1.80, +3.28) belly
```

Note the left/right pair is point-symmetric (both z signs flipped), not mirrored â€”
...
`large_fireball`, â€¦) are `PUFFER_STATE` definitions â€” full schema in
[effects.md](effects.md).

## Weapons, damage, and AI keys

The airframe half of the combat data â€” of which only `turrets` is consumed by the remake so
...
**`weapons`** â€” a list of 5-tuples `[weapon_id, count, ?, ?, range]`, ids into
[weapons.md](weapons.md). On `player_airplane` it is a **capability catalogue, not a
loadout**: all 39 buyable ids at once (`wep_00`â€“`15`, `25`/`27`/`28`, and the full
`wep_30`â€“`73` player matrix), each with position-5 range `10000` â€” a UI sentinel, since the
real per-plane loadout is executable-resident. On an AI def it is the actual armament:
`bloodhawk` = `[["wep_04",4,â€¦,800],["wep_07",2,â€¦,800],["wep_00",9000,â€¦,900]]` â€” a carried
count, two undecoded factors, then an **engagement range** in metres (800â€“900 for AI
fighters, 500 for the boat/truck). Positions 3â€“4 are inferred, not confirmed.

**`cannon_jam`** (`player_airplane`) â€” `heat_safe_limit 1000`, `heat_dissipation_rate 50`,
`jam_chance 0.1`; reads as a gun-overheating model paired with `FIRING_HEAT` in
[weapons.md](weapons.md). âš  **Dead data: the original executable has no reader for it**
...
**`armor` / `health`** â€” the AI two-pool damage model (fighters `64/64`â€¦`100/100`, always
equal; `patrolboat`/`t_truck` `0/40`, unarmoured soft targets). Carried by the 12 base aircraft
defs plus the boat and truck â€” 15 in all. `PlaneStats` does not read either. Armour is spent
before health, the same ordering as the per-part pools.

âš  **The whole-vehicle pair and the per-part pools are not alternatives â€” 11 defs resolve both.**
An `r*` AI variant chains to its base def (`rbloodhawk â†’ bloodhawk â†’ basic_airplane`), so it
inherits `armor 64` *and* carries its own 4Ã—20/20 `destroyable_parts`. No **player** def resolves
a whole-vehicle pair at all (`pbloodhawk â†’ player_airplane â†’ basic_airplane` carries none in the
chain), so for player planes the per-part pools are the whole model.

âš  **Those 11 both-resolvers are the `r*` remote-player family, and no roster spawns one** â€” so no
...
kill can arrive with zones still healthy â€” every zone exhausted is sufficient, not necessary;
...
that, an **aircraft or autogyro that is not the player** draws a fresh uniform **Â±5 %** on both
...
both: three placed `ptboat1`â€“`3` at the refinery, and 12 roster boats in M05. Which model applies is
...
**`turrets`** â€” on 16 defs: the five player turret airframes (`pavenger`, `pbalmoral`,
...
(`balmoral_turret0`â€“`3`). A turret entry carries a title and a node and nothing else â€” the
...
airframe def â€” a census settles which:
...
- **0 of the 12 player defs carry or inherit it â€” including all five turret airframes**
...
Â±11Â° is the AI's fixed-gun firing tolerance. The design's gunnery model backs the reading â€” an
NPC's Dead Eye statistic sets the radius of a lead sphere it will shoot into.

**`bullethole_anims`** â€” per player plane, the ON_CALL cockpit-glass hit-decal anims
`bullet1`â€¦`bullet5` (see [anim-definitions.md](anim-definitions.md)).

**`mode`** â€” the dynamics class, and the one key that decides which AI behaviour an aircraft flies.
...
fallback for the roster's `pref_engage_alt` slot. âš  It is a **weight on the evasive-maneuver draw**,
...
`talker`, `constitution`, `accentID`) â€” the same nine-stat vector the mission rosters author
...
liveries â€” see [paint.md](paint.md). A few airframe oddballs round out the set: `fuel`,
...
during crash recovery. âš  **The def struct holds them in roll/pitch/yaw order** (`+0x264`â€¦`+0x278`)
...
`ai_input_limit_pitch` (11 defs, 0.79â€“0.91) and one `ai_input_limit_yaw` (0.79); the rest inherit
...
them is fixed at `0`â€¦`111.76 m/s` (250 mph) for every airframe and has no token at all.
...
(`0x47afc0`â€“`0x47b081`; classes 0 and 4 fly the aeroplane path, 1 the autogyro path, 2 the ground
...
The class is what gates the per-spawn Â±5 % jitter of eleven runtime slots â€” `fd_speed`,
`ThrustFactor`, `drag_factor`, `pitch_torque`, `roll_torque`, the two `rates` and two `turns`
values, and the whole-vehicle armour/health maxima â€” which runs on classes 0 and 1 only, for
