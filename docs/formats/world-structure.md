...
Part of the [format documentation](READoE.md) (see also [gamez.md](gamez.md), [clutter.md](clutter.md), [fogvol.md](fogvol.md) — the `fvol*` volumes named below are the ambient cloud field's authored extent). ooved from CLAUDE.md on 2026-07-18.

- World content lives in TWO places: the `world1` World node's `children` (66 in C1: horizon, cloud groups, zeppelins, trains, fvol volumes…) **and** ~350 top-level parentless subtrees referenced only via the World's `partitions` (12×12 spatial grid over `area` x,z ∈ [-12288, 0]; each cell lists node indices; the union of distinct refs = placed terrain tiles + buildings + vehicles). The `area` record's fields map as `left` = xoin, `right` = xoax, `top` = zoin, `bottom` = zoax (right-handed Y-up world coords — "top" is the smaller z).
...
- `horizon` subtree = original skydome (~8.8 km radius, verts centered on the **origin** while the world area is x,z ∈ [-12288, 0] — the engine must translate it with the viewer; it's a camera-anchored backdrop, not scenery). Two day/night zone variants as child nodes: `zone1` (day) = `h_zone1scroll` (sky2.tif tiling haze dome, u tiles ~22× around, sunset-orange vertex tints near the horizon; its top is a **flat untextured 12-gon cap** at dome-local y 2792.8, painted the zone's own `FOG_COLOR` — ⚠ **authored, not unfinished**: every dome in the install is a textured wall plus flat `FOG_COLOR` cap/skirt faces, so below the horizon the dome *is* the fog wall the terrain fades into. The old "unfinished, likely never player-visible" reading was removed from `WorldBuilder.BuildHorizon`'s doc in `PLAN-overcast-match` `B18`; `B16` then found the cap/skirt geometry is a hard seam in 5 of 8 chapters once `fog: false` is honoured — visible, and previously only camouflaged by `ForceFogged` painting it the same colour as its surroundings) + `o28` (flat-colored gray sea-haze skirt, y −2000..271); `zone2` (night) = `moon` (moon1.tif quad; background painted (66,73,99) ≈ the night-sky tint, engine color-keys and billboards it) + `g1155` (gray below-horizon skirt + **Sky1.tif-textured cap** tinted night-blue 64,72,100) + `h_zone2scroll` (Sky1.tif walls with a dusk gradient: warm 249,213,189 at the horizon → dark blue at top) + `stars` (**no polygons — its 64 stars live in the mesh's `lights` array** as point-sprite lights: gray-white 219, all at y 1301 spread over ±5.9 km, params 0.17/30/4000/6000 undecoded). **No mission ever names its zone, and that is not a gap in the readers** — decompiled 2026-08-09 (`zone_id` bullet below): zone selection is a *per-frame camera-state* filter, not a per-mission choice, so there was never a field to find. A node draws iff its `zone_id` is `−1` or matches the camera's own weather state (1 below the cloud deck / 2 above it / 3 inside a `fog_zone`). **User observation (2026-07-14): the original always loads the C1 airfield at night — so zone2 is the sky the game shows at that mission's altitude.** ⚠ That does **not** make `zone1` test-only or never player-visible — the inference this bullet used to draw is disproven: in every deck chapter `zone1` is exactly what the player flies under, the ceiling seen below the cloud deck (`docs/formats/weather.md`, the `B14` landing and its un-retirement of the `zone1` `FOG_ALTITUDE` note). **The sky UVs do scroll, and only C1's do** (settled 2026-08-09): `h_zone1scroll`'s model carries `texture_scroll` **0.07 u/s in u**, matching `support\c1\tex_fx.gw`'s `FindNode h_zone1scroll` + `Object3DSetScroll on 0.07 0.0` — the boot-script value is already baked into the shipped model, so the two sources agree and there is no second scroll path. Rendered since 2026-07-22 (`SceneBuilder.EffectiveScroll` → a `scroll_rate` uniform → `texture(albedo_tex, UV + scroll_rate * csky_time)`) and visible since the zone-1 dome started being built at all (`WorldBuilder.DomeZonesToBuild`, `zone_id`-gated). ⚠ **The name lies elsewhere:** C4's sole zone-1 node is *named* `h_zone2scroll` but its `texture_scroll` is **0**, and C1C/C2B author no zone-1 scroll — a reused name, not a scroll. Asserted by `CSVo.Tests/HorizonDomeTests.cs` `OnlyC1sZoneOneDomeScrollsAndItScrollsAtTheAuthoredRate`.
- **`zone_id` — every gamez node carries one (decoded 2026-07-22; runtime meaning decompiled and
  ...
  `oech3.ZoneGate` per camera. Note `zone_id` does *not* explain the C5 coarse/fine ground pair —
...
- **`zone_set` — every polygon carries one (decoded 2026-08-04, `BL-057`, parse+census only, no
  ...
  **Not a ground selector — checked and ruled out** (the original backlog evidence): this is *not*
  ...
  gate reads the per-polygon `zone_set`** — nothing in `CSVo/src` reads `ZoneSet` either, and per
...
- **Authoring gizmos: a lone untextured triangle is a marker, never scenery (decoded 2026-08-01).**
  oission and AI anchors are mesh-less empty nodes, but many carry one **single-polygon,
