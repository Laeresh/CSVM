...
`wep_02` (50slug, `ANIMATION large_fireball`) routes `buildings` → `bld_damage.flt`, which no install
...
- **A reader/control def** (`3040slug_gunhit`, `he_ground_effect`, `large_fireball`) or an
  **undefined** name (`bld_damage.flt`, …) names no root, so nothing instances and a stand-in
  ...
  `buildings`(11) whose bound name reaches neither a gamez root nor the world-effects runtime →
  nothing, as in the original, whose parse leaves `bld_damage.flt`'s slot null (see the
  unresolved-names table). A name the runtime **does** carry, which on `buildings` means only
  `wep_02`'s `large_fireball`, renders.
...
|---|---|---|---|
| `bld_damage.flt` | gun `IMPACT` `buildings` | IMPACT | defined by no install file; the original's parse leaves the slot null and draws nothing ([../org/weaponImpact.md](../org/weaponImpact.md)) |
...
a gamez model root, so `ProjectilePool` instances nothing for them (no crash). A gun's
`bld_damage.flt` takes no stand-in either, since the original draws nothing for it.
