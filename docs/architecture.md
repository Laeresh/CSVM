...
⚠ Stand-ins/stubs, all named in the class comment: host DEATH is wired for ZEPPELIN hosts only
  (`HostDied(node)`, fed by `ZeppelinRuntime.ZeppelinKilled` — F18); fixed-installation hosts
  still have no death source, and the door choreography is skipped (both F20).
