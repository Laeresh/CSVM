...
| `F15` | step the map-edge block depth: 1 → 2 → 3 → 4 → 12 → 1, plus whatever `--map-edge-block=` asked for. Rebuilds the whole window, so it hitches (4–34 ms, one frame); the HUD reports how long it took. No-op while F14 is hidden (`BL-105`) |
