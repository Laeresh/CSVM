...
| `Esc` | | at the launchscreen: back, and quit from the Mode screen. In flight it opens the pause board instead — a board menu's Exit item is what leaves a session, so a pad can reach it too. An armed cell on the Controls screen is the exception: Escape there is written in as the control, the way the original writes it into one, and the pad's B is what abandons a capture |
