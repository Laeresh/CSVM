...
| click | | weapon lab: park the held plane on the surface under the cursor, at the panel's stand-off, nose on it — `--weapon-click=x,y`, or `--weapon-target=x,y,z` / `--weapon-surface=water\|buildings\|dirt` to place without a mouse at all |
...
| `F15` | step the map-edge block depth: 1 → 2 → 3 → 4 → 12 → 1, plus whatever `--map-edge-block=` asked for. Rebuilds the whole window, so it hitches; the HUD reports how long it took. No-op while F14 is hidden (`BL-105`) |
| `F16` | swap the map-edge fold between `mirror` and `repeat` — `--map-edge-mode=`. Repeat is a **refuted** hypothesis (`CAP-17`) kept to be looked at, and it steps at every seam. No-op while F14 is hidden |
