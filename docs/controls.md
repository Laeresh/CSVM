...
| `F8` | | cycle the cockpit views: Cockpit ↔ Nose, entering Cockpit from the chase view — `--view=cockpit` / `--view=nose`. The original's own binding for "Cycle Cockpit Views" (`OriginalScreenshots/Keybinds Views 1.png`) |
| `F6` | | select the chase view, the way back out of the first-person pair — `--view=chase` (the default). The original selects each view separately rather than cycling all three, and binds "Access Chase View" to `F7` (same screenshot); `F6` is this port's choice because `F7` is recorded in `docs/org/cameraViews.md` as the flyby camera, which is a contradiction nobody has settled yet |
...
| RMB-held mouse | right stick | first-person free-look: the head pans at the decoded 2 rad/s in whichever direction the input points. Direction only, so a light stick deflection pans as fast as a hard one — the original's input is a hat switch. RMB-held matches the freecam's look posture; the mouse does nothing in flight otherwise. The original reaches the two look modes through key selectors instead (`K` Access Snap Look Mode, `J` Access Smooth Look Mode) rather than by which device moved, which is a filed item |
