...
4. ☑ Launchscreen — Mode → Chapter → Plane, keyboard/controller navigation, no-args default *(done 2026-07-19; `src/UI/LaunchMenu.cs` + `RunGame.ps1`; PlaneViewer build refactored into a re-runnable `StartSession`/`ReturnToMenu` — the first in-process world rebuild. Verified: bare launch shows the menu (no plane built), Mode/Plane screens render + stats load, CLI fly/stunt/orbit/static regressions unchanged, scripted C1-stunt→C4-free in-process rebuild clean. Open: interactive keyboard-only/controller-only playtest, font/placement TUNE)*
5. ☑ Splitscreen foundation — shared-world multi-viewport rendering + per-player input (CLI-driven, free flight) *(done 2026-07-19; `src/UI/SplitScreen.cs` + `src/Flight/PlayerRig.cs`, `--players=N`. Per-player sky/deck/puffs/whiteout on a reserved visual-layer band (17–20) with matching camera cull masks; MapEdgeExtender window = union of all players' rings; per-player pad binding + HUD pane + 1/√N audio gain; `--hold` gained `|` per-player sequences. Verified: 2P/4P panes, planes 9.1 km apart, one player 6.2 km off the map edge over extension terrain while the other shows its own moonlit dome, P2 crash leaves P1 flying, CLI battery unchanged, no measurable frame cost 1P/2P/4P on C1+C5 to 4K (vsync-limited). Fixed two pre-existing bugs found here: the straight-and-level Slerp freeze in FlightModel, MarkerHud's zero-size font spam. Open: per-player HUD metrics (item 6), multi-pad playtest, weaker-GPU perf number)*
6. ☑ Splitscreen join + plane select — Start-to-join in the launchscreen, simultaneous pick, 2P/4P layouts, per-player HUD *(done 2026-07-19; `src/UI/MenuInput.cs` + `src/Flight/HudMetrics.cs` new, LaunchMenu rebuilt around a player list. Two-phase join: P1 (keyboard + every unclaimed pad — a set, not `pads[0]`, so the phantom-device fix survives) picks Mode/Chapter and the pad it steers them with is claimed for P1; joining then opens on the aircraft screen only, so Start unambiguously means "a new player". Join strip on every screen; all players pick simultaneously and lock with A (duplicates allowed), B unlocks/leaves. The **aircraft screen splitscreens** with 2+ players — one panel each via the shared `SplitScreen.PaneRect` (same geometry as the flight panes), own roster/cursor/stats, border lights when locked. Per-player planes through `PlaneFor(i)` + a per-plane stats cache; `--plane=a,b` is the CLI equivalent. HUD scale = window height damped by sqrt(pane share) — the identity in 1P, so the regression is provable: 1P menu pixel-identical, static orbit byte-identical, 1P fly/stunt differ in 4/16 sampled px (0 in HUD bands) vs a 13,346-px noise floor. Same-turn fixes: bottom-anchored gauge dials, scoreboard fits a pane, throttle on its own line below full screen, content-aware menu layout scale. `--debug-join=N` screenshots the split select at 2P/3P/4P. **Open: the menu→multi-player launch is unverified at runtime (needs two controllers) — join/un-join, lock race, per-pad binding, Esc-from-splitscreen owed a user playtest; pane HUD sizes TUNE)*
7. ☑ Splitscreen stunt race — per-player progress/timers, shared end scoreboard *(done 2026-07-19; `src/Flight/StuntRace.cs` + `src/Flight/StuntRaceBoard.cs` new. Every player races an independent copy of the zone list (`StuntMission.ForAnotherPlayer` — parsed once per session), with their own marker HUD, clock and progress; finishing locks that pilot's placing and time while they continue flying with a "waiting for N pilots…" line, and the last finisher raises a full-window ranked board (placing / colour-coded tag / aircraft / zones / total + gap to winner, winner in their identity colour). R = rematch (restarts every player), gated until everyone is in. Verified: 2P/4P boards ranked correctly with four different aircraft; the decisive mid-race shot shows P1 finished 1st while P2 still flies with its own 0:00.9 clock, 0/5 zones and its own marker; a scripted 2P straight-line run 60 m apart completed dz2 for P1 only (0/5 vs 1/5 in the panes) proving progress is per-player; an auto-rematch soak looped 5 races with byte-identical totals each time (no state leak) and no errors. Solo path unchanged (splits board + `ScoreStore` best time preserved at 2:02.8); free-flight 1P/2P, orbit and a no-dzone chapter at 2P all clean. Open: user playtest with two controllers — board fonts/placement, BL-083 closed 2026-08-11: a finished pilot keeps flying while their race state is frozen, rematch feel)*
