...
**Active plans:** [`docs/PLAN-M5-polish-7.md`](docs/PLAN-M5-polish-7.md), ten items in five waves, `A1`, `B11`, `C22` and `D31` landed. Runs 5 and 6, the scrapbook and the campaign co-op are archived in [`docs/plans/plans.md`](docs/plans/plans.md).
Next: `C21`, then `B12`, then wave `E`; `A2` and run 6's `BL-546` and `BL-535`, with CM13's race pace and CM08's `BL-597`, are owed at the controls.
