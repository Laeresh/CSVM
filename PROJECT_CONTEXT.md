...
**Active plans:** [`docs/PLAN-menu-presentations.md`](docs/PLAN-menu-presentations.md), every item landed and `E44` open on the at-the-controls rows, [`docs/PLAN-M5-polish-8.md`](docs/PLAN-M5-polish-8.md) at Wave D, and [`docs/PLAN-M5-polish-9.md`](docs/PLAN-M5-polish-9.md) at Wave D, every code item landed or disproven.
Next: the menu plan's at-the-controls pass `E44` lists (each mode flown and returned in both presentations, a campaign mission won and lost, a real pad, the sounds and pointer), then it moves to `docs/plans/`; polish-8 `D31` and polish-9 `D31` fly in one sitting, consuming `PT-97` to `PT-117`; `BL-535` and CM13's race pace remain owed at the controls, as does the enhanced mode's pass against `docs/plans/PLAN-enhanced-graphics.md`'s Open judgements.
