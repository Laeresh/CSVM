...
**Active plans:** [`docs/PLAN-M5-polish-5.md`](docs/PLAN-M5-polish-5.md), every item landed and verified on the plan branch, and [`docs/PLAN-M5-polish-6.md`](docs/PLAN-M5-polish-6.md), all eleven items resolved on `worktree-m5-polish-6`; [`docs/PLAN-scrapbook.md`](docs/PLAN-scrapbook.md) is at `A1` and `A2` landed on `worktree-bl-622-debrief-decode`.
Next: `D31`, the CM04 to CM09 (plus CM13) sortie at the controls, then close run 5; run 6's two partials are `B11` (`BL-546`) and `B13` (`BL-535`); the scrapbook's Wave A continues at `A3`.
