...
**Active plan:** [`docs/PLAN-public-release.md`](docs/PLAN-public-release.md), the public release, its `A1` triage next.
Next: `BL-755` to `BL-769` carry run 13's sortie, Plane Construction first (`BL-706`, `BL-758` to `BL-763`, `BL-769`); `BL-746` to `BL-754` carry run 12's, the second pilot's screen first; `BL-714` on a moving hull, `BL-739`; `BL-700` and `BL-702` carry run 10's sortie; `BL-693` and `BL-696` carry what input rebinding left open; `BL-699` carries the wave-spawn hitch; `BL-535`, CM13's race pace and the enhanced mode's pass against `PLAN-enhanced-graphics`'s Open judgements are owed at the controls.
