...
**Active plan:** [`docs/PLAN-m3-polish-5.md`](docs/PLAN-m3-polish-5.md), Wave B. **Next:** `B5`
(`BL-061` item 3); owed cockpit re-tests (`PT-13`–`PT-26`) remain in [`playtest.md`](playtest.md). Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
