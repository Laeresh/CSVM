...
**No active plan.** [`PLAN-targeting.md`](docs/plans/PLAN-targeting.md) completed 2026-08-16 (11 items, Waves A–D): player-selected targeting is live in `--fly`, scripted with `--target=`, drawn through `TargetHud` with the original's marker and `--debug-markers`' health/armor read. Run **`/new-plan`** to start the next one; until then, `backlog.md` names the open items.
