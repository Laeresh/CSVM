...
**Active plan:** [`docs/PLAN-menu-presentations.md`](docs/PLAN-menu-presentations.md), every item landed and `E44` open on the at-the-controls rows.
Next: `BL-701` (an exempted surface renders far brighter than the original), `BL-700` and `BL-702` carry polish run 10's closing sortie; `BL-693`, `BL-696` and `BL-697` carry what input rebinding left open; the menu plan's at-the-controls pass `E44` lists (each mode flown and returned in both presentations, a campaign mission won and lost, a real pad, the sounds and pointer), then it moves to `docs/plans/`; `BL-699` carries the wave-spawn hitch; `BL-535` and CM13's race pace remain owed at the controls, as does the enhanced mode's pass against `docs/plans/PLAN-enhanced-graphics.md`'s Open judgements.
