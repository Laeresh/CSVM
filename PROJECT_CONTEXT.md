...
**Active plan:** [`docs/PLAN-deepening.md`](docs/PLAN-deepening.md) — Waves A, B and C complete, D
one item from it.
**Next:** `D11` (`OwesBounce` assertions in the `bounce-launch` suite) and `E13`
(`IEmitter`/`IEmitterFactory` + extract `EmitterDirector`, against the interface `E12` settled).
