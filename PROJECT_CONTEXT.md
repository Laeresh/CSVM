...
shareable build for friends who own the game; A1/A2/B11/B12 landed, B13 zip Sandbox-proven
(`PLAN-m3-polish-8` completed 2026-08-05, indexed in [`docs/plans/plans.md`](docs/plans/plans.md)).
**Next:** user reviews `packaging/README.md` and does the first hand-off closing the plan; then
the `PT-39` gun-line/splash cockpit flight, the owed cockpit re-tests (`PT-26`–`PT-28`,
`PT-31`–`PT-37`, [`playtest.md`](playtest.md)), or `BL-262`. Verify any change with
