...
**Active plan: [`docs/PLAN-ai-damage-and-engine-audio.md`](docs/PLAN-ai-damage-and-engine-audio.md)** — every item landed; the owed at-the-controls playtest is the last gate. The ordnance runtime (`BL-406`) is complete: [`docs/plans/PLAN-ordnance-types.md`](docs/plans/PLAN-ordnance-types.md), its decode [`docs/org/ordnanceTypes.md`](docs/org/ordnanceTypes.md), with `PT-74`(c)/`PT-75`/`PT-76` living on as ordinary playtest rows.

**Next:** fly the AI damage/engine-audio playtest, then move that plan to `docs/plans/` with its `COMPLETE` banner and close `BL-385`.
