...
**Active plans:** [`docs/PLAN-menu-presentations.md`](docs/PLAN-menu-presentations.md), every item landed and `E44` open on the at-the-controls rows, [`docs/PLAN-M5-polish-8.md`](docs/PLAN-M5-polish-8.md) at Wave D, every code item landed or disproven, and [`docs/PLAN-M5-polish-10.md`](docs/PLAN-M5-polish-10.md) with `B12` and `D31` open.
Next: `BL-693`, `BL-696` and `BL-697` carry what input rebinding left open; the menu plan's at-the-controls pass `E44` lists (each mode flown and returned in both presentations, a campaign mission won and lost, a real pad, the sounds and pointer), then it moves to `docs/plans/`; polish-8 `D31` owes `PT-102` re-flown, then it moves too; `BL-535` and CM13's race pace remain owed at the controls, as does the enhanced mode's pass against `docs/plans/PLAN-enhanced-graphics.md`'s Open judgements.
