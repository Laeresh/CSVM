...
**Active plans:** [`docs/PLAN-M5-polish-12.md`](docs/PLAN-M5-polish-12.md), the menu seam, `E41`'s sortie owed; [`docs/PLAN-M5-polish-13.md`](docs/PLAN-M5-polish-13.md), its leftovers, ten items landed, `E41`'s sortie owed.
Next: `BL-714` on a moving hull, `BL-739`; `BL-700` and `BL-702` carry run 10's sortie; `BL-693` and `BL-696` carry what input rebinding left open; `BL-706` is the menu finding run 13 left out; `BL-699` carries the wave-spawn hitch; `BL-535`, CM13's race pace and the enhanced mode's pass against `PLAN-enhanced-graphics`'s Open judgements are owed at the controls.
