...
- `src/Utils/` (7) — session-wide services: clock, log, seed, shader time, config, startup profile, graphics mode. Determinism lives here.
...
The day-to-day 30 of 146 — 146 is `docs/cli.md`'s flag-index count, kept equal to the parser's accepted-flag count on purpose. **[`docs/cli.md`](docs/cli.md) opens with an index of all of them, grouped**, and each flag's bullet there is the **description of record** — the whole `--debug-*` family, the paint overrides, spawn/mission selection, scripted `--hold` input, the data-path overrides, and the deprecated `--campos`/`--spawn-at`/`--spawn-dir` spellings of the placement pair.
...
**Active plans:** [`docs/PLAN-menu-presentations.md`](docs/PLAN-menu-presentations.md) at Wave A, [`docs/PLAN-M5-polish-8.md`](docs/PLAN-M5-polish-8.md) at Wave D, and [`docs/PLAN-M5-polish-9.md`](docs/PLAN-M5-polish-9.md) at Wave A.
Next: menu `A1` inventories every in-scope menu journey and its original evidence; polish-8 `D31` flies the landed items; polish-9 `A1` re-confirms CM02's hook swing; `BL-535` and CM13's race pace remain owed at the controls, as does the enhanced mode's pass against `docs/plans/PLAN-enhanced-graphics.md`'s Open judgements.
