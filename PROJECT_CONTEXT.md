...
- `src/Utils/` (9) — session-wide services: clock, log, seed, shader time, config, startup profile, options, graphics mode. Determinism lives here.
- `src/Testing/` (6) — the in-engine assertion harness behind `--run-tests` and the `--dump-*` probes.
- `src/Bindings/` (15) — the input binding model and the named-action seam: device identity, the tagged control, the binding list an action resolves through, the registry that resolves a device identity to a live pad, a player's action map resolved once per tick, and the shipped default keymap with its versioned per-player file.
...
**Active plans:** [`docs/PLAN-menu-presentations.md`](docs/PLAN-menu-presentations.md), every item landed and `E44` open on the at-the-controls rows, [`docs/PLAN-M5-polish-8.md`](docs/PLAN-M5-polish-8.md) and [`docs/PLAN-M5-polish-9.md`](docs/PLAN-M5-polish-9.md) at Wave D, every code item landed or disproven, and [`docs/PLAN-M5-polish-10.md`](docs/PLAN-M5-polish-10.md) with `B11`/`B12` and `D31` open, and [`docs/PLAN-input-rebinding.md`](docs/PLAN-input-rebinding.md) at Wave B, with `A1`-`A4` and `C21` landed.
Next: input-rebinding owes a reconciliation of Wave B's three private device-state copies plus the axis rescale, then `B14`; the menu plan's at-the-controls pass `E44` lists (each mode flown and returned in both presentations, a campaign mission won and lost, a real pad, the sounds and pointer), then it moves to `docs/plans/`; polish-8 `D31` and polish-9 `D31` fly in one sitting, consuming `PT-97` to `PT-117`, and polish-10's `B11`/`B12` land only after they do; `BL-535` and CM13's race pace remain owed at the controls, as does the enhanced mode's pass against `docs/plans/PLAN-enhanced-graphics.md`'s Open judgements.
