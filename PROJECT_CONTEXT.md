...
Next: polish-7's `A1`, then `A2`; run 6 handed back `BL-546` and `BL-535`, and CM13's race pace with CM08's `BL-597` are owed at the controls. The plane selection run left `BL-658` (the ammo screen's second description pane) and `BL-659` (no aid opens a combo box).
