...
**Active plans:** [`docs/PLAN-menu-presentations.md`](docs/PLAN-menu-presentations.md), Waves A to E landed, `F51` open (Original's Game Options page carrying both option rows) and `E44` open on the at-the-controls rows, and [`docs/PLAN-M5-polish-8.md`](docs/PLAN-M5-polish-8.md) at Wave D.
Next: the menu plan's `F51`, then the at-the-controls pass `E44` lists (each mode flown and returned in both presentations, a campaign mission won and lost, a real pad, the sounds and pointer), then it moves to `docs/plans/`; polish `D31` flies the landed items; `BL-535`, `BL-597` and CM13's race pace remain owed at the controls, as does the enhanced mode's pass against `docs/plans/PLAN-enhanced-graphics.md`'s Open judgements.
