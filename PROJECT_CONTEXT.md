...
Next: the menu plan's at-the-controls pass `E44` lists (each mode flown and returned in both presentations, a campaign mission won and lost, a real pad, the sounds and pointer), then it moves to `docs/plans/`; polish `D31` flies the landed items; `BL-535`, `BL-597` and CM13's race pace remain owed at the controls.
