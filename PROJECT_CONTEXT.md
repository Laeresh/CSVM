...
**Active plans:** [`docs/PLAN-M5-polish-7.md`](docs/PLAN-M5-polish-7.md), ten items in five waves, none started, and [`docs/PLAN-plane-selection.md`](docs/PLAN-plane-selection.md), ten items in four waves, waves A and B done and wave C open at its last item. Runs 5 and 6, the scrapbook and the campaign co-op are archived in [`docs/plans/plans.md`](docs/plans/plans.md).
Next: polish-7's `A1`, then `A2`; plane selection's `C9`; run 6 handed back `BL-546` and `BL-535`, and CM13's race pace with CM08's `BL-597` are owed at the controls.
