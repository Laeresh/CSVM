...
- `backlog.md` — unscheduled work, ordered by theme (Damage & destruction, Weapons & combat, …): every item one flat `BL-NNN` bullet with a type tag (`[Bug]`/`[Feature]`/`[Research]`/`[Tuning]`/`[Cleanup]`) and an optional status tag (`[Owed-playtest]`, `[Blocked: <blocker>]`). The TUNE list is the set of items tagged `[Tuning]`. Move items into a plan when scheduled; **delete when landed — a `FIXED`/closed entry does not stay here.** Its record belongs in `docs/HISTORY.md`; its traps in `docs/verification.md` or `docs/architecture.md`. **If closing it leaves follow-up work, that follow-up becomes its own new entry with a `⚠ Traps` section** naming the rejected fixes and the misleading instruments — an open thread buried inside a section headed `FIXED` is invisible to anyone scanning for work.
...
(items 1–6); Wave C (`C7`–`C10`) is open, sequential, one golden-moving item per landing window.
**Next:** `C7` (`BL-262`), then `C8`/`C9`/`C10`; also owed: the `PT-39` gun-line/splash cockpit
flight, `PT-40` belly-slide splash check, and the cockpit re-tests (`PT-26`–`PT-28`,
