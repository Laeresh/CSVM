...
**Active plans:** none. Polish run 7 is archived with runs 5 and 6, the scrapbook, the campaign co-op and the plane selection screen in [`docs/plans/plans.md`](docs/plans/plans.md); its `A2`, `E41` and `E42` landed partial, each with its backlog entry rewritten to what remains.
Next: `BL-628` (the docking hook's repeated animation) came back open from run 7's sortie, and `BL-648` now has a second suite carrying its shard-order defect, which is the one red in a full gate. Run 6's `BL-546` and `BL-535`, CM13's race pace and CM08's `BL-597` are still owed at the controls, and the plane selection run left `BL-658` (the ammo screen's second description pane) and `BL-659` (no aid opens a combo box).
