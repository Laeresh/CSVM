...
**Active plan: [`docs/PLAN-targeting.md`](docs/PLAN-targeting.md)** — player-selected targeting, on branch `worktree-targeting`. Waves A and B done (targeting is live in `--fly`, scripted with `--target=`); C21 split the marker into its own `TargetHud`, in its final home for C22. Next: **C22**.
