...
**Active plans:** none. Archived plans are indexed in [`docs/plans/plans.md`](docs/plans/plans.md).
Next: `BL-648` is the one red in a full gate, with two suites carrying its shard-order defect. `BL-628` (the docking hook's repeated animation), `BL-546`, `BL-535`, `BL-597` and CM13's race pace are owed at the controls; `BL-658` (the ammo screen's second description pane) and `BL-659` (no aid opens a combo box) are open.
