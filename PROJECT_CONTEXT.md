...
Next: `BL-764` and `BL-765` carry what runs 12 and 13's sorties left open, Plane Construction first; `BL-693` carries what input rebinding left open; `BL-699` carries the wave-spawn hitch; CM13's race pace, `PT-119`'s two CM14 sorties and the enhanced mode's pass against `PLAN-enhanced-graphics`'s Open judgements are owed at the controls.
