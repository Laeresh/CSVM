...
**Active plan: [`docs/PLAN-targeting.md`](docs/PLAN-targeting.md)** — player-selected targeting, on branch `worktree-targeting`. Waves A and B done (targeting is live in `--fly`, scripted with `--target=`); A1's decode is in [`docs/org/targeting.md`](docs/org/targeting.md) and C22 is unblocked. Next: **C21**.
