...
**No active plan.** [`docs/plans/PLAN-clutter-uv-placement.md`](docs/plans/PLAN-clutter-uv-placement.md)
completed 2026-08-10 (all 11 items ☑), unmerged on `worktree-clutter-uv-placement`. Run
**`/new-plan`** to start the next one; until then, `backlog.md` names the open items — `BL-305`
(C5 packing, improved but still open) and `BL-337` (`far_fade_range`, freshly deferred) are what
