...
**Active plans:** none. Runs 5 and 6, the scrapbook and the campaign co-op are archived in [`docs/plans/plans.md`](docs/plans/plans.md).
Next: choose the next run from `backlog.md`; run 6 handed back `BL-546` and `BL-535`, and CM13's race pace with CM08's `BL-597` are owed at the controls.
