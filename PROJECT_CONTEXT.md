...
**Next:** `C6` (give `Log` a settable console sink) and `D8` (grill the `MotionSet` interface — no
code) are next in sequence; `E13` (`IEmitter`/`IEmitterFactory` + extract `EmitterDirector`) is
unblocked since `E12` landed. [`docs/PLAN-weapon-lab.md`](docs/PLAN-weapon-lab.md) and `BL-245`
