...
**Active plan: [`docs/PLAN-m3-polish-4.md`](docs/PLAN-m3-polish-4.md)** (10 items, waves A–D; written 2026-08-01). **Next:** `D10` — `BL-086`, explosive radius + proximity fuse. Owed cockpit re-tests (`PT-13`–`PT-26`) remain in [`playtest.md`](playtest.md). Verify any change with **`.\RunTests.ps1`** (build → units → in-engine suites → golden hashes → one exit code); read [`docs/verification.md`](docs/verification.md) before measuring anything.
