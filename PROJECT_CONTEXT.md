...
**Active plan: [`docs/PLAN-targeting.md`](docs/PLAN-targeting.md)** — player-selected targeting, on branch `worktree-targeting`. Waves A, B and C are done (targeting is live in `--fly`, scripted with `--target=`, drawn through `TargetHud` with the original's marker, `--debug-markers`' health/armor read, and a pinned golden). Next: **D31**, the plan's close-out.
